...
class MoviePlayerPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # ── Scrub bar ────────────────────────────────────────────────
        scrub_bar = tk.Frame(self, bg=SCRUB_BG, height=34)
        ...
        if HAVE_EDIT:
            self.marker_bar = MarkerBar(
                ...
                seek_to   = lambda f: self._loader.request(self.current_path, f),
    ...
    def _start_playback(self):
        ...
        if self._player_mode == "timeline" and self.combine_strip and self.combine_strip.get_clips():
            ...
            self._play_all_mode = False
        # Always resume from _frame_index.  After EOS, _on_playback_frame(-1)
        # has already reset _frame_index to in_point, so a fresh play starts
        # from the clip beginning without any wrapping logic here.
        self._playing = True
        self._loader.cancel()
        ...
        if edit_list and edit_list.has_cuts and self._total_frames > 0:
            # Build a cut-aware WAV so audio matches the edited video exactly.
            # Video engine starts immediately; audio joins when the WAV is ready
            # (near-instant if full WAV is already cached, otherwise a few seconds).
            segments   = edit_list.kept_segments(self._total_frames)
            snap_path  = self.current_path
            snap_fps   = self._fps

            def _on_audio_ready(wav_path):
                # Guard: user may have stopped or moved to another file
                if not self._playing or self.current_path != snap_path:
                    return
                if wav_path:
                    # _audio._wav_path and _audio._ready are already set by prepare_edited
    ...
    def destroy(self):
        """Stop audio and release resources when the panel is destroyed."""
        self._audio.stop()
        super().destroy()
    ...
    def _scrub_jump(self, x: int):
        ...
        if self._total_frames <= 0:
            return
        slider_half = 8
        usable = max(getattr(self, 'scrub', None) and self.scrub.winfo_width() - slider_half * 2 or 400, 1)
        x = max(0, min(x - slider_half, usable))
        frame = int(round(x / usable * (self._total_frames - 1)))
