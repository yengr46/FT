...
def _render_pdf_first_page(path: str, sz: int, longpath_func=None) -> Image.Image:
    """Render the first page of a PDF to a PIL RGB image."""
    try:
        import fitz  # PyMuPDF
    except Exception as e:
        raise RuntimeError("PyMuPDF is required for PDF thumbnails. Install with: pip install pymupdf") from e
    ...
    try:
        ...
        if rect.width <= 0 or rect.height <= 0:
            ...
            zoom = min(max(sz / rect.width, sz / rect.height), 6.0)
            zoom = max(0.5, zoom)
...
def _render_docx_preview(path: str, sz: int, longpath_func=None) -> Image.Image:
    """Render DOCX by asking ft_office_preview for a preview PDF, then render its first page."""
    try:
        ...
        try:
            ...
            raise RuntimeError("DOCX thumbnail support requires ft_office_preview.py") from e

    preview_pdf = get_office_preview_pdf(path)
    if not preview_pdf or not os.path.exists(preview_pdf):
        raise RuntimeError("DOCX preview PDF was not created")
    return _render_pdf_first_page(preview_pdf, sz, longpath_func=None)


def make_thumb_from_path(path: str, sz: int, longpath_func=None) -> Image.Image:
    """Open path and return a fitted RGB thumbnail image.

    JPG/JPEG files are opened with Pillow.
    PDF files are rendered from their first page using PyMuPDF.
    DOCX files are routed through ft_office_preview.py to create/render a preview PDF.
    """
    ext = os.path.splitext(str(path))[1].lower()

    if ext == ".pdf":
        img = _render_pdf_first_page(path, sz, longpath_func=longpath_func)
    elif ext == ".docx":
        img = _render_docx_preview(path, sz, longpath_func=longpath_func)
    else:
...
def get_thumbnail(path: str, sz: int, longpath_func=None):
    ...
    """
    try:
        img = make_thumb_from_path(path, sz, longpath_func=longpath_func)
