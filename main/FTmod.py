"""
FTmod.py  —  FTView-style layout refactor
-
Version: FT41 DB cleanup 29-Apr-2026 + tree startup fix 06-May-2026
Combined photo and PDF tagging utility.
Toggle between Photos and Documents with the 📷/📄 button.

Architecture:
  Photos root / Documents root  — originals, NEVER modified
...
from libraries.ft_startup_check import check_startup_requirements

_OPTIONAL_STARTUP_MISSING = check_startup_requirements(
    "FTmod 2.0",
    required_libraries=[
        ("Pillow", "PIL", "pip install Pillow"),
        ("PyMuPDF", "fitz", "pip install pymupdf"),
        ("NumPy", "numpy", "pip install numpy"),
        ("fpdf2", "fpdf", "pip install fpdf2"),
    ],
    required_helpers=[
        "libraries.ft_gps",
        "libraries.ft_widgets",
        "libraries.ft_tooltip",
        "libraries.ft_thumbs",
        "libraries.ft_panels",
        "libraries.ft_thumb_layout",
        "libraries.ft_viewer",
        "libraries.ft_zoom",
        "libraries.ft_db",
        "libraries.ft_thumb_cache",
        "libraries.ft_contactsheet",
        "libraries.ft_pdf_ops",
        "libraries.ft_file_ops",
        "libraries.ft_startup",
    ],
    optional_libraries=[
        ("tkintermapview", "tkintermapview", "GPS map window only"),
    ],
    show_optional_warning=False,
)


# ── GPS / EXIF helpers (extracted) ─────────────────────────────────────────────
from libraries.ft_gps import _get_gps_coords, _scan_folder_for_gps
from libraries.ft_widgets import TREE_LEFT_W, FileCountTree, FolderTreeWidget, TREE_COL_W, TREE_WIDTH, TREE_SCROLL_W, TREE_PAD_R, TREE_TREE_COL_EXTRA_W, show_file_sort_menu, _sort_btn_label

# ── Tooltip (extracted) ───────────────────────────────────────────────────────
from libraries.ft_tooltip import _Tooltip, _tip
from libraries.ft_thumbs import (
    scale_to_fit as _thumb_scale_to_fit,
    make_placeholder as _thumb_placeholder,
    fit_text as _thumb_fit_text,
    get_thumbnail as _thumb_get_thumbnail,
    make_thumb_jpeg_from_path as _thumb_make_jpeg_from_path,
    jpeg_bytes_to_image as _thumb_jpeg_to_image,
)
from libraries.ft_panels import make_main_panel, make_split_panel
from libraries.ft_thumb_layout import calculate_thumb_layout, print_thumb_layout
...
try:
    from libraries.ft_movie import make_movie_thumbnail_fast as _make_video_thumb
    HAVE_VIDEO_THUMB = True
except ImportError:
    HAVE_VIDEO_THUMB = False
...
PDFS_ROOT    = ""
VIDEOS_ROOT  = ""
# Flat root list (new model) — list of (path, label) tuples, ordered.
ROOTS: list = []
# Legacy typed lists — kept as compat shims until UI refactor (Task 12).
PHOTOS_ROOTS: list = []
PDFS_ROOTS:   list = []
VIDEOS_ROOTS: list = []
...
PDF_MERGE_QUEUE = "PDF Merge Queue"
...
def _parse_roots(ini_path, section):
    ...
    with open(ini_path, 'r', encoding='utf-8', errors='replace') as f:
        for line in f:
            ...
            roots.append((os.path.normpath(path), name))
    return roots
...
def _projects_ini_path():
    return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Projects.ini")

def _load_ft_categories():
    """Load FTCategories.json from script directory. Returns dict or empty structure."""
    p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "FTCategories.json")
...
def _load_projects():
    """Parse Projects.ini via ft_projects. Returns {name -> {path, roots}}."""
    import sys as _sys, os as _os
    _lib = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))), "libraries")
    if _lib not in _sys.path:
        _sys.path.insert(0, _lib)
    from ft_projects import load_projects as _lp
    return _lp(_projects_ini_path())

def _save_projects(projects):
    """Write projects dict to Projects.ini via ft_projects (root_N format)."""
    import sys as _sys, os as _os
    _lib = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))), "libraries")
    if _lib not in _sys.path:
        _sys.path.insert(0, _lib)
    from ft_projects import save_projects as _sp
    _sp(_projects_ini_path(), projects)
...
def _activate_project(name, proj):
    ...
    global _ACTIVE_PROJECT, _DB_PATH, ROOTS
    ...
    # New flat roots list
    ROOTS = list(proj.get('roots', []))

    # Backward-compat shims: typed lists point at all roots; single scalars at first root.
    first = ROOTS[0][0] if ROOTS else ""
    PHOTOS_ROOTS = ROOTS[:]
    PDFS_ROOTS   = ROOTS[:]
    VIDEOS_ROOTS = ROOTS[:]
    PHOTOS_ROOT  = first
...
def _create_project(name, proj_path, photos_roots=None, pdfs_roots=None,
                    videos_roots=None, roots=None):
    """Create a new FTProj_ folder structure and register in Projects.ini.

    Accepts roots=[(path, label)] (new) or photos_roots/videos_roots/pdfs_roots
    (legacy — merged in order: photos, videos, pdfs).
    """
    ...
    if roots is None:
        # Merge legacy typed lists in order
        seen = set()
        roots = []
        for entry in (photos_roots or []) + (videos_roots or []) + (pdfs_roots or []):
...
def _active_project_name_runtime():
    """Return the active project name from memory or FileTagger.ini.

    Settings can be opened after the project selector has already activated a
    project, but FileTagger.ini may have been rewritten by older builds without
    active_project.  In that case _ACTIVE_PROJECT is the reliable source.
    """
    try:
        n = (_ACTIVE_PROJECT or {}).get('name', '')
        if n:
            return n
    except Exception:
        pass
    return _get_active_project_name()


def _load_config():
    global PHOTOS_ROOT, PDFS_ROOT, VIDEOS_ROOT, THUMB_SIZE, PHOTOS_ROOTS, PDFS_ROOTS, VIDEOS_ROOTS
    global PROC_TEMP_FOLDER, _DB_PATH, _INI_PATH, ROOTS
    ini = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "FileTagger.ini")
    _INI_PATH = ini
    ...
    if active_name:
        # Migrate old photos/videos/pdfs keys to root_N on first open
        try:
            import sys as _sys2, os as _os2
            _lib2 = _os2.path.join(_os2.path.dirname(_os2.path.dirname(_os2.path.abspath(__file__))), "libraries")
            if _lib2 not in _sys2.path:
                _sys2.path.insert(0, _lib2)
            from ft_projects import migrate_project_file as _mpf
            _mpf(_projects_ini_path())
        except Exception as _me:
            print(f"ft_projects migration error: {_me}")
        projects = _load_projects()
    ...
    if PDFS_ROOTS:
        ...
        PDFS_ROOTS = [(PDFS_ROOT, os.path.basename(PDFS_ROOT.rstrip('/\\')) or "Documents")]
    VIDEOS_ROOTS = _parse_roots(ini, "videos")
    if VIDEOS_ROOTS:
        VIDEOS_ROOT = VIDEOS_ROOTS[0][0]
    else:
        VIDEOS_ROOT = cfg.get("videos", "root", fallback=VIDEOS_ROOT)
        if VIDEOS_ROOT:
            VIDEOS_ROOTS = [(VIDEOS_ROOT, os.path.basename(VIDEOS_ROOT.rstrip('/\\')) or "Videos")]
    # Also populate flat ROOTS from legacy typed lists
    seen = set()
    ROOTS = []
    for entry in PHOTOS_ROOTS + VIDEOS_ROOTS + PDFS_ROOTS:
...
def _ui_path(p):
    """Return a normal display/selection path; keep \\?\ only for file I/O."""
    if not p:
        return p
    p = str(p)
    if p.startswith('\\\\?\\'):
...
# ── Mode configuration ─────────────────────────────────────────────────────────
def _mode_cfg(mode):
    """Return a config dict for the given filter mode.

    root is set to first accessible ROOTS entry (or ""); caller may override.
    """
    root = ROOTS[0][0] if ROOTS else (PHOTOS_ROOT or "")
    if mode == "photos":
        return dict(root=root, exts=PHOTO_EXTS, label="Photos", icon="📷",
                    file_word="image", col_head="JPGs")
    elif mode == "videos":
        return dict(root=root, exts=VIDEO_EXTS, label="Videos", icon="🎬",
                    file_word="video", col_head="Videos")
    elif mode == "all":
        return dict(root=root, exts=PHOTO_EXTS | VIDEO_EXTS | DOCUMENT_EXTS,
                    label="All", icon="📁",
                    file_word="file", col_head="Files")
    else:
        return dict(root=root, exts=DOCUMENT_EXTS, label="Documents", icon="📄",
                    file_word="document", col_head="Docs")

_FileTagger_instance = None  # set in __init__

# ── SQLite database connection ────────────────────────────────────────────────
# FT41: connection state and schema ownership now live entirely in ft_db.py.
# FT.py keeps only small compatibility wrappers used by the existing UI code.

def _db():
    """Return the active FileTagger SQLite connection owned by ft_db.py."""
    return ft_db.get_connection()

def _db_default_path():
    """Return default DB path: <script_dir>\Database\FileTagger.db"""
    return ft_db.default_db_path(__file__)
...
def db_open(path=None):
    """Open/create the SQLite DB through ft_db.py."""
    global _DB_PATH
    if path is None:
        path = _DB_PATH if _DB_PATH else _db_default_path()
    conn = ft_db.open_database(
        path,
        default_path=_db_default_path(),
        ini_path=(_INI_PATH or os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "FileTagger.ini")),
    )
    _DB_PATH = path
    return conn

def db_close():
    """Close the database connection owned by ft_db.py."""
    ft_db.close_database()

# ── Thumbnail cache wrappers — global per-PC cache (ft_thumb_cache) ──────────
# Paths are normalised via _ui_path (strips \\?\ prefix, os.path.normpath)
# before being passed to the cache so keys are consistent across sessions.

def thumb_get(source_path):
    return _ft_thumb_cache.get_thumb(_ui_path(source_path))

def thumb_get_many(source_paths):
    # Build normalised→original map so the caller gets back its original keys.
    norm_to_orig = {_ui_path(p): p for p in source_paths}
    result = _ft_thumb_cache.get_thumb_many(list(norm_to_orig.keys()))
    return {norm_to_orig.get(k, k): v for k, v in result.items()}

def thumb_put(source_path, jpeg_bytes):
    _ft_thumb_cache.put_thumb(_ui_path(source_path), jpeg_bytes)

def thumb_put_many(items):
    # items: (path, jpeg) or (path, jpeg, width, height)
    _ft_thumb_cache.put_thumb_many(
        [(_ui_path(item[0]), *item[1:]) for item in items]
    )
...
def thumb_move(source_paths, dest_folder):
    for old in (source_paths or []):
        new = _ui_path(os.path.join(dest_folder, os.path.basename(old)))
        _ft_thumb_cache.move_thumb(_ui_path(old), new)
...
def _photo_cache_get(path, sz, source_mode="stored"):
    key = (path, sz, source_mode)
    if key in _photo_cache:
        _photo_cache.move_to_end(key)   # mark as recently used
        return _photo_cache[key]
    return None

def _photo_cache_put(path, sz, photo, source_mode="stored"):
    key = (path, sz, source_mode)
...
def _get_pdf_info(path):
    """Return page count for PDFs and DOCX preview PDFs."""
    if not HAVE_FITZ:
        return 0, ""
    try:
        open_path = path
        ...
        if ext in DOCX_EXTS:
            try:
                ...
                open_path = get_office_preview_pdf(path)
                use_longpath = False
            except Exception as e:
                return 0, str(e)
        doc = fitz.open(_longpath(open_path) if use_longpath else open_path)
...
# ── Collection DB wrappers (delegated to consolidated ft_db.py) ───────────────

def _list_collections(root=None):
    return ft_db.list_collections(_db(), root)

def _read_collection(name, root=None):
    return ft_db.read_collection(_db(), name, root)

def _write_collection(name, root=None, tagged=None, tagged_at=None, order=None):
    return ft_db.write_collection(_db(), name, root, tagged, tagged_at, order)

def _delete_collection(name, root=None):
    return ft_db.delete_collection(_db(), name, root)

def _append_collection_items(name, root=None, paths=None):
    return ft_db.append_collection_items(_db(), name, paths or [], root)
...
# ── Cull list DB wrappers (delegated to consolidated ft_db.py) ────────────────

def _read_cull_list(root=None):
    return ft_db.read_cull_list(_db(), root, exists_func=os.path.exists)

def _write_cull_list(root=None, culled=None, culled_at=None):
    return ft_db.write_cull_list(_db(), root, culled, culled_at)


# ── First-run migration from .txt files → DB ───────────────────────────────────


# ── Startup/build helpers (extracted to ft_startup.py) ────────────────────────
_get_build_id = ft_startup.get_build_id
_get_build_timestamp = ft_startup.get_build_timestamp
_get_build_string = ft_startup.get_build_string
_show_startup_splash = ft_startup.show_startup_splash
_check_libraries = ft_startup.check_libraries
_show_library_warning = ft_startup.show_library_warning

# Backward-compatible display variable for older labels.
BUILD_DATE = _get_build_string()


# ── First-run migration wrapper (delegated to consolidated ft_db.py) ──────────
def _migrate_txt_to_db(root):
    return ft_db.migrate_txt_to_db(
        _db(),
        root,
        ft_system_dir=FT_SYSTEM_DIR,
        coll_subdir=COLL_SUBDIR,
...
class FileTaggerTree(FileCountTree):
    ...
    Three columns:
        ...
        Tree   — matching files in child/subordinate folders only, not this folder
    ...
    def __init__(self, parent, extensions=None,
                 ...
                 col_files="Files", col_tagged="Tree", col_thumbs="Thumbs",
                 **kw):
        ...
        kw['columns'] = [
            ...
            (col_tagged, TREE_COL_W + TREE_TREE_COL_EXTRA_W, "e"),
            (col_thumbs, TREE_COL_W, "e"),
        ]
        # Skip FileCountTree.__init__ column injection because FTmod needs
        # three columns, not FileCountTree's standard two.  But keep the
        # FileCountTree async-count state because its public refresh/set_root
        # methods now depend on these attributes.  Without this, set_root()
        # raises AttributeError(_count_generation), _init_mode aborts, and the
        # folder tree stays empty.
    ...
    def _fill_node(self, path, *, skip_tree_col=False):
        """Fill all three columns for a single node.

        Columns:
            Files  = current folder only        (fast: single scandir)
            Tree   = subordinate folders only   (slow: os.walk — deferred by default)
            Thumbs = current folder only        (fast: indexed DB query)

        skip_tree_col=True leaves the Tree column as-is, avoiding the os.walk.
        The background filler calls this with skip_tree_col=False to fill it later.
        """
        ...
        if not skip_tree_col:
            tree_n = self._get_tagged_count(path)
            self.set_col(path, 1, f"({tree_n})" if tree_n > 0 else "-")
    ...
    def _fill_children_of(self, path, *, skip_tree_col=False):
        """Fill columns for direct children of path."""
        for child in self._tree.get_children(path):
            if self.PLACEHOLDER not in child:
                self._fill_node(child, skip_tree_col=skip_tree_col)

    def _fill_tree_col_bg(self, nodes):
        """Fill the Tree column for a list of node paths in a background thread.

        tkinter is not thread-safe: the worker thread must NEVER call any
        tkinter method directly.  Results are posted to a queue and applied
        by _poll_tree_col_queue() which runs entirely on the main thread.
        """
        import threading
        import queue

        if not hasattr(self, '_tree_col_queue'):
            self._tree_col_queue = queue.Queue()

        def _worker():
            for path in nodes:
                try:
                    n = self._get_tagged_count(path)
                    self._tree_col_queue.put((path, n))
                except Exception:
                    pass
            self._tree_col_queue.put(None)  # sentinel: batch done

        t = threading.Thread(target=_worker, daemon=True)
        t.start()
        # Start polling on the main thread (idempotent if already polling)
        self._poll_tree_col_queue()

    def _poll_tree_col_queue(self):
        """Drain the tree-column result queue on the main thread.

        Called via after() so it always runs on the main thread.  Re-schedules
        itself until the sentinel (None) is received.
        """
        if not hasattr(self, '_tree_col_queue'):
            return
        try:
            processed = 0
            while processed < 20:  # drain up to 20 items per tick
                item = self._tree_col_queue.get_nowait()
                if item is None:
                    return  # sentinel — batch complete, stop polling
                path, n = item
                if self._tree.exists(path):
                    self.set_col(path, 1, f"({n})" if n > 0 else "-")
                processed += 1
        except Exception:
            pass
        # More items may be coming — poll again after a short delay
        self._tree.after(50, self._poll_tree_col_queue)

    # ── Overrides ──────────────────────────────────────────────────────────────

    def _populate_root(self, path):
        """Populate root, fill Files+Thumbs columns immediately, Tree column in background."""
        super(FileCountTree, self)._populate_root(path)
        # Fill Files and Thumbs synchronously — both are fast.
        # Skip the Tree column (os.walk per node) to avoid blocking the UI.
        self._fill_children_of(path, skip_tree_col=True)
        # Fill Tree column for visible nodes in a background thread.
        nodes = [c for c in self._tree.get_children(path)
                 if self.PLACEHOLDER not in c]
        if nodes:
            self._fill_tree_col_bg(nodes)

    def _on_node_open(self, path):
        """Expand node, fill Files+Thumbs immediately, Tree column in background."""
    ...
    def refresh_stats(self):
        """Refresh Files+Thumbs columns for all visible nodes synchronously.
        Tree column (os.walk) is refreshed in background to avoid freezing."""
        all_nodes = []
        def _collect(node):
            for child in self._tree.get_children(node):
                if self.PLACEHOLDER not in child:
                    self._fill_node(child, skip_tree_col=True)
                    all_nodes.append(child)
                    _collect(child)
        _collect("")
        if all_nodes:
            self._fill_tree_col_bg(all_nodes)


class FileTagger(FTZoomMixin):
    ...
    def __init__(self, root=None):
        ...
        self._root         = root
        self.mode          = "all"    # filter mode: all|photos|videos|pdfs
        self._active_root_idx = 0      # index into ROOTS list
        ...
        self._all_files    = []
        self._page_num     = 0
        self._sort_column  = "name"   # "name" or "date_taken"
        self._sort_reverse = False
        self._auto_cache   = False
        self.use_stored_thumbs = None  # set to BooleanVar in _build_ui after tk.Tk() exists
        ...
        self._last_click_idx    = None    # index of last selection-anchor cell for Shift+click range
        self._last_select_action = "select"  # Shift+click repeats last Ctrl select/deselect action
    ...
    def _display_thumb_render_size(self, fallback_size=None):
        """Return render size for screen-resolution display thumbnails.

        When Use Stored Thumbs is OFF, this deliberately renders from source
        larger than the stored 250x250 DB cache so PDFs/DOCXs become readable.
        """
        try:
            sz = int(fallback_size or self._disp_size or THUMB_SIZE)
        except Exception:
            sz = THUMB_SIZE

        try:
            if not self.use_stored_thumbs.get():
                # Use the actual display image size where available, but force
                # a noticeable quality jump above the stored 250 cache.
                layout = getattr(self, "_thumb_layout", None)
                if layout is not None:
                    sz = max(sz, int(getattr(layout, "image_w_px", sz) or sz))
                sz = max(sz, 1400 if getattr(self, "mode", "") == "pdfs" else 650)
        except Exception:
            pass

        return max(64, min(1400, int(sz)))


    def _render_document_thumb_direct(self, path, sz):
        """Render PDF/DOCX thumbnail directly from the source file.

        Used only for Documents mode when Use Stored Thumbs is OFF.
        It deliberately bypasses stored DB blobs and any in-memory thumbnail cache.
        """
        img, ok, _err = _thumb_get_thumbnail(path, sz, longpath_func=_longpath)
        if ok and img is not None:
            return img
        return _thumb_placeholder(sz)

    def _on_use_stored_thumbs_changed(self):
        """Toggle displayed thumbnail source and redraw current page.

        ON  = use DB stored 250x250 thumbs where available.
        OFF = bypass DB blobs and render from source at display/screen size.
        """
        try:
            _photo_cache.clear()
        except Exception:
        ...
        try:
            ...
            pass
        try:
            self._status("Displayed thumbs: stored DB cache" if self.use_stored_thumbs.get() else "Displayed thumbs: screen render")
        except Exception:
            pass
        try:
            self._show_page()
    ...
    def _init_mode(self):
        """Initialise or reinitialise for the current mode/filter and active root."""
        self.mode_cfg = _mode_cfg(self.mode)
        # Override root with the actually selected root (not just ROOTS[0])
        if ROOTS:
            idx = min(getattr(self, '_active_root_idx', 0), len(ROOTS) - 1)
            self.mode_cfg['root'] = ROOTS[idx][0]
        ...
        offline_roots = [name for path, name in ROOTS if not _path_accessible(path)]
        online_roots  = [(path, name) for path, name in ROOTS if _path_accessible(path)]
        if offline_roots:
        ...
        if root_accessible:
            try:
                ...
                if PDF_MERGE_QUEUE not in cols:
                    _write_collection(PDF_MERGE_QUEUE, root_dir, set(), {})
                self._refresh_collection_list()
    ...
    def _update_root_combobox(self):
        """Populate the root selector combobox from the flat ROOTS list."""
        try:
            labels = []
            for rpath, rname in ROOTS:
                basename = os.path.basename(rpath.rstrip('/\\')) or rpath
                label = rname if (rname and rname != basename) else self._root_display_label(rpath)
                if not _path_accessible(rpath):
                    label = f"[OFFLINE] {label}"
                labels.append(label)
            self.root_cb['values'] = labels
            idx = getattr(self, '_active_root_idx', 0)
            if labels:
                self.root_cb.current(min(idx, len(labels) - 1))
        except Exception: pass
    ...
    def _on_root_select(self):
        ...
        try:
            ...
            if idx < 0 or idx >= len(ROOTS):
                return
            rpath, _rname = ROOTS[idx]
            if rpath == self.mode_cfg.get('root'):
                return
            if not _path_accessible(rpath):
                ...
                self._update_root_combobox()
                return
            self._active_root_idx = idx
            self._switch_root(rpath)
        except Exception as e:
            print(f"_on_root_select error: {e}")

    def _switch_root(self, new_root):
        """Switch to a different root folder (mode/filter unchanged)."""
        self._save_current_collection()
    ...
    def _build_ui(self, root=None):
        ...
        self.use_stored_thumbs = tk.BooleanVar(master=self.win, value=True)
        # Ensure tk.StringVars are always created (they need tk.Tk() to exist)
        if self.coll_var is None:
            self.coll_var = tk.StringVar()
        if self._jump_var is None:
            self._jump_var = tk.StringVar()
        self.win.overrideredirect(False)
        self.win.title("FTmod version 2.6")
        ...
        # ── Filter buttons (replaces mode combobox) ────────────────────
        self._mode_var = tk.StringVar(value="All")  # kept for compat
        self._mode_switching = False
        tk.Label(mode_folder_frame, text="Show:", bg=_mf_bg, fg=_lbl_fg,
                 font=("Segoe UI",9,"bold")).pack(side="left", padx=(6,2))
        self._filter_btns = {}
        _filter_defs = [("all","All"), ("photos","Photos"),
                        ("videos","Videos"), ("pdfs","Docs")]
        for _fm, _fl in _filter_defs:
            _is_active = (_fm == self.mode)
            _fb = tk.Button(
                mode_folder_frame, text=_fl,
                bg="#888888" if _is_active else _mf_bg,
                fg="#ffffff" if _is_active else _fg,
                font=("Segoe UI", 8), relief="flat", bd=1,
                padx=5, pady=1, cursor="hand2",
                command=lambda m=_fm: self._set_filter(m))
            _fb.pack(side="left", padx=1, pady=3)
            self._filter_btns[_fm] = _fb
        ...
        def _thumbs_extras(parent, bg):
            chk = tk.Checkbutton(
                parent,
                text="Use Stored Thumbs",
                variable=self.use_stored_thumbs,
                command=self._on_use_stored_thumbs_changed,
                bg=bg,
                fg="#111111",
                activebackground=bg,
            ...
            chk.pack(side="left", padx=(6, 2), pady=2)
        self._tb_extras["thumbs"] = _thumbs_extras

        tk.Label(cols_size_frame, text="Columns:", bg=_cs_bg, fg=_cs_fg,
                 font=("Segoe UI",9,"bold")).pack(side="left", padx=(6,1))
        ce = tk.Spinbox(cols_size_frame, from_=1, to=30, width=3,
                        textvariable=self._cols_var,
                        font=("Segoe UI",9), justify="center",
                        command=lambda: self._on_grid_dims_change('cols'))
        ce.pack(side="left", padx=(0,6))
        ce.bind("<Return>", lambda e: self._on_grid_dims_change('cols'))
        ce.bind("<FocusOut>", lambda e: self._on_grid_dims_change('cols'))
        ...
        def _coll_extras(parent, bg):
            ...
            self.btn_sort = tk.Button(parent, text=_sort_btn_label(self._sort_column, self._sort_reverse),
                      command=self._show_sort_menu,
                      bg="#2255aa", fg="white", font=("Segoe UI", 8, "bold"),
                      relief="flat", padx=5, pady=3, cursor="hand2",
                      activebackground="#3366bb", activeforeground="white")
        ...
        _tb_defs = [
            ...
            ("view",      "View",      PANEL_BG,row2_centre,"view",[
                ("Located",   "#1a6655"),("Map","#1a5577"),("FTVideo","#3a1a6a"),("Folders","#445566"),
                ("Similar",   "#5a3a6a"),
                ("Compress",  "#3a3a6a"),
                ("⊞ 2-Panel", "#2a4a6a"),
            ]),
            ...
            ("cache",     "Thumbs",    PANEL_BG,row2_centre,"thumbs",[
                ("Generate", "#226655"),
        ...
        _btn_cmds = {
            ...
            "FTVideo":       lambda: self._launch_ftvideo_from_selection(),
        ...
        _btn_tips = {
            ...
            "FTVideo":       "Open selected video files in FTVideo player",
        ...
        # ── Folder tree — FileTaggerTree from ft_widgets ──────────────────────
        self._tree_widget = FileTaggerTree(
            left_paned,
            extensions=self.mode_cfg['exts'],
            get_tagged_count=self._count_tree_files_below,
            get_thumb_count=self._count_thumbs,
            on_select=self._on_tree_path_selected,
            on_delete_folder=self._on_tree_folder_deleted,
            on_folders_changed=self._on_tree_folders_changed,
            show_root_entry=False,
            bg=BG
        )
        self._tree_widget._ft_app = self
        left_paned.add(self._tree_widget, minsize=200,
                       width=TREE_LEFT_W, stretch='always')  # widened with Tree column
        ...
        # Folder selection is handled through FileTaggerTree(on_select=...) so
        # shared widget changes cannot swallow FT thumbnail loading.
        ...
        # ── FTmod viewer area: File list | Thumbnails | Image preview ────────
        # Keep the existing FT thumbnail canvas intact inside the middle panel so
        # all ornaments (Mark, Rotate, Zoom, selection watermarks) continue to be
        # drawn by the original FT code.
        self._viewer_paned = tk.PanedWindow(paned, orient="horizontal", bg=BG,
                                            sashwidth=6, sashrelief="groove",
                                            handlesize=8, sashpad=2)
        paned.add(self._viewer_paned, minsize=500, stretch="always")

        self._filelist_panel = tk.Frame(self._viewer_paned, bg=BG2)
        self._viewer_paned.add(self._filelist_panel, minsize=160, width=260, stretch="never")
        fl_hdr = tk.Frame(self._filelist_panel, bg="#1a3a5c", height=26)
        fl_hdr.pack(side="top", fill="x")
        fl_hdr.pack_propagate(False)
        tk.Label(fl_hdr, text="  Files", bg="#1a3a5c", fg="white",
                 font=("Segoe UI", 9, "bold"), anchor="w").pack(side="left")
        self._filelist_sort_btn = tk.Button(
            fl_hdr, text=_sort_btn_label(self._sort_column, self._sort_reverse),
            font=("Segoe UI", 9, "bold"), bg="#1a3a5c", fg="white",
            relief="flat", cursor="hand2",
            activebackground="#2255aa", activeforeground="white",
            command=lambda: self._show_sort_menu(self._filelist_sort_btn),
        )
        self._filelist_sort_btn.pack(side="right", padx=(0, 20))
        fl_body = tk.Frame(self._filelist_panel, bg=BG2)
        fl_body.pack(fill="both", expand=True)
        self.file_list = tk.Listbox(fl_body, bg="white", fg="#111111",
                                    font=("Segoe UI", 9), activestyle="none",
                                    selectbackground=TREE_SEL_BG, selectforeground="white",
                                    exportselection=False, relief="flat", bd=0,
                                    highlightthickness=0)
        self.file_list.pack(side="left", fill="both", expand=True)
        self.file_list.bind("<<ListboxSelect>>", self._on_file_list_select)
        self.file_list.bind("<Button-1>", self._on_file_list_mouse_down)
        self.file_list.bind("<Double-Button-1>", lambda e: self._zoom_current_filelist_item())
        fl_sb = tk.Scrollbar(fl_body, orient="vertical", command=self.file_list.yview)
        fl_sb.pack(side="right", fill="y")
        self.file_list.configure(yscrollcommand=fl_sb.set)

        right = tk.Frame(self._viewer_paned, bg=BG)
        self._viewer_paned.add(right, minsize=260, stretch="always")

        # ── Shared FTView image viewer panel ────────────────────────────────
        # Do not create a second set of image controls here. FTmod uses the
        # same ViewerPanel as FTView so rotate/open/fit buttons and mid-side
        # navigation chevrons remain identical across both apps.
        self._preview_panel = tk.Frame(self._viewer_paned, bg=BG2)
        self._viewer_paned.add(self._preview_panel, minsize=220, width=430, stretch="never")
        ...
        # Backward-compatible aliases for older FTmod helper methods.
        self._preview_path = None
        self._preview_photo = None

        # ── View status bar — top of thumbnail panel ──────────────────────────
        _vs_bg = "#c8d8c8"
        _vs_fg = "#111111"
        ...
        # In 2-panel mode both panels are shown side by side, but thumbnail
        # metrics remain the same shared FTView calculation.
    ...
    def _mode_var_changed(self, *args):
        """Legacy trace stub — filter buttons replaced the mode combobox."""
        pass

    def _toggle_mode(self):
        """Legacy stub — use _set_filter() instead."""
        pass

    def _set_filter(self, mode):
        """Switch the file-type display filter without rebuilding the tree or changing root.

        mode: "all" | "photos" | "videos" | "pdfs"
        """
        if mode == self.mode:
            return
        if mode == "pdfs" and not HAVE_FITZ:
            if not messagebox.askyesno("PyMuPDF not installed",
                ...
                "Continue in Documents mode without thumbnails?",
                parent=self.win):
                return

        self.mode = mode
        # Update extension set in mode_cfg (keep current root)
        current_root = self.mode_cfg.get('root', '')
        self.mode_cfg = _mode_cfg(mode)
        self.mode_cfg['root'] = current_root

        # Sync tree widget extension set so Files column counts match the new mode
        try:
        ...
        # Re-scan current folder with new extension filter — tree stays as-is
        if getattr(self, 'current_folder', '') and os.path.isdir(self.current_folder):
    ...
    def _update_filter_buttons(self):
        ...
        try:
            _mf_bg = "#cccccc"
            _fg    = "#111111"
            for fm, btn in getattr(self, '_filter_btns', {}).items():
                active = (fm == self.mode)
                btn.config(bg="#888888" if active else _mf_bg,
                           fg="#ffffff" if active else _fg)
        except Exception:
            pass
    ...
    def _rename_collection(self):
        ...
        def on_ok():
            ...
            try:
                if _db() is not None:
                    ft_db.rename_collection(_db(), self.collection, new_name)
    ...
    # ── Folder tree ────────────────────────────────────────────────────────────
    def _thumb_count_under_folder(self, path):
        """Return cached thumbnail count under folder, robust to UI/long-path and slash variants.

        Cache rows may have been written by older builds using a normal path, a
        Windows long-path prefix, upper/lower drive letters, or mixed slashes.
        Tree refresh must count them all so newly generated thumbnails appear in
        the Thumbs column immediately.
        """
        if _db() is None:
            return 0
        try:
            base = _ui_path(path)
            base = os.path.normpath(base).replace('/', '\\').rstrip('\\/')
            if not base:
                return 0
            patterns = [base + '\\%']
            if not base.startswith('\\\\?\\'):
                patterns.append('\\\\?\\' + base + '\\%')
            where = ' OR '.join(["lower(replace(path,'/','\\')) LIKE lower(?)" for _ in patterns])
            return ft_db.thumb_count_under_folder(_db(), path, ui_path_func=_ui_path)
        except Exception as e:
            print(f"_thumb_count_under_folder error: {e}")
            return 0

    def _folder_stats(self, path):
        exts = self.mode_cfg['exts']
        path = os.path.normpath(_ui_path(path))
        try:
            entries = [e.name for e in os.scandir(_longpath(path))
        ...
        if file_n > 0:
            thumb_n = self._thumb_count_under_folder(path)
            if thumb_n > 0:
    ...
    def _count_tagged(self, path):
        ...
        try:
            return sum(
                ...
                and os.path.normpath(_ui_path(e.path)) in self.tagged
            )
        except Exception:
            return 0

    def _count_thumbs(self, path):
        """Callback for FileTaggerTree — count thumbnails cached for this folder only.

        This is direct-folder only. It deliberately does not count thumbnails
        from child folders.
        """
        try:
            return ft_db.thumb_count_in_folder(_db(), path, ui_path_func=_ui_path)
        except Exception:
            return 0
    ...
    def _is_contact_sheets_folder(self, folder=None):
        ...
        Contact sheets are PDFs even when FTmod is currently in Photos mode,
        so _ContactSheets must be loaded with PDF_EXTS.
        """
    ...
    def _display_exts_for_folder(self, folder=None):
        if self._is_contact_sheets_folder(folder):
            return PDF_EXTS
        return self.mode_cfg.get('exts', PHOTO_EXTS | VIDEO_EXTS | DOCUMENT_EXTS)

    def _display_file_word_for_folder(self, folder=None):
        return 'PDF' if self._is_contact_sheets_folder(folder) else self.mode_cfg.get('file_word', 'image')

    def _count_tree_files_below(self, path):
        """Count matching files in child/subordinate folders only.

        This backs the folder tree's Tree column:
            Files  = matching files directly in this folder
            Tree   = matching files below this folder, excluding this folder
            Thumbs = cached thumbnails directly in this folder
        """
        path = os.path.normpath(_ui_path(path))
        exts = self._display_exts_for_folder(path)
        total = 0
        try:
            for root, dirs, files in os.walk(_longpath(path)):
                root_ui = _ui_path(root)
                if os.path.normcase(os.path.normpath(root_ui)) == os.path.normcase(path):
                    # Exclude the selected/current folder itself.
                    continue
                for name in files:
    ...
    def _has_direct_files(self, path):
        """Return True if path directly contains files for the current mode."""
        exts = self.mode_cfg['exts']
        path = os.path.normpath(_ui_path(path))
        try:
            return any(
                e.is_file() and os.path.splitext(e.name)[1].lower() in exts
                for e in os.scandir(_longpath(path))
            )
        except Exception:
            return False
        ...
        try:
            for entry in os.scandir(_longpath(path)):
                if entry.is_file() and os.path.splitext(entry.name)[1].lower() in exts:
                    return True
                if entry.is_dir():
                    if self._has_files_below(_ui_path(entry.path)):
                        return True
        except: pass
        return False
    ...
    def _on_tree_folder_deleted(self, folder_path):
        ...
        Keep UI, DB thumbnail cache and current-folder state consistent.
        """
        folder_path = _ui_path(folder_path)

        # Remove thumbnail cache rows for the deleted folder/subtree.
        try:
            ft_db.delete_thumbs_for_folder(_db(), folder_path)
        except Exception:
            pass

        # Clear in-memory thumbnail cache for the folder/subtree.
        try:
        ...
        # If the deleted folder, or a child of it, was being displayed, enter a
        # real empty state instead of leaving stale thumbnails visible.
        try:
            cur = _ui_path(getattr(self, 'current_folder', ''))
            deleted_prefix = folder_path.rstrip("\\/") + os.sep
            if cur == folder_path or cur.startswith(deleted_prefix):
                # Do not leave the app in a fake empty-folder state for a path
                # that no longer exists.  Move the active folder to the real
                # parent after the structural tree refresh completes.
                self._pending_folder_after_tree_change = os.path.normpath(os.path.dirname(folder_path))
                self._all_files = []
        ...
        # Remove stale tree node if it still exists.
        # iids are now normalised via _ui_path() in _populate_level, so a
        # single normpath lookup is sufficient.
        try:
            if self.tree.exists(folder_path):
                self.tree.delete(folder_path)
        except Exception:
            pass

        # Refresh counts for the parent node only.
        # Do NOT call refresh_stats() — it walks every visible node and calls
        # _count_tree_files_below (os.walk) on each, freezing the UI.
        try:
            parent_dir = os.path.normpath(os.path.dirname(folder_path))
            if parent_dir and self.tree.exists(parent_dir):
                self._tree_widget._fill_node(parent_dir)
                self._tree_widget._fill_children_of(parent_dir)
        except Exception:
            pass
    ...
    def _collapse_node(self, iid):
        try:
            ...
            if self._has_subdirs(iid):
                self.tree.insert(iid, "end", iid=iid + getattr(self._tree_widget, "PLACEHOLDER", "__ph__"), text="")
    ...
    def _do_tree_open(self):
        def _check(iid):
            ...
            phs = [c for c in ch if "__ph__" in c]
    ...
    def _on_tree_path_selected(self, path):
        """Folder selected from shared FileTaggerTree.

        This is the primary FT navigation path.  It deliberately receives the
        selected folder path from ft_widgets instead of depending on a separate
        Treeview event binding, because the shared tree widget owns selection
        events for all apps.
        """
        try:
            path = _ui_path(path)
            ...
            already_open = False
            try:
                already_open = bool(self.tree.item(path, "open"))
            except Exception:
                pass
            collapsing = path in self._closing_nodes

            # If this folder has no direct files but has files somewhere below,
            # jump down to the first populated descendant.
            no_direct_files = not self._has_direct_files(path)
            if no_direct_files and not collapsing and self._has_files_below(path):
            ...
            if no_direct_files and already_open and not collapsing:
    ...
    def _on_tree_select(self, event):
        """Backward-compatible event handler; delegates to path-based handler."""
        try:
            sel = self.tree.selection()
            if not sel: return
            self._on_tree_path_selected(sel[0])
        except Exception as _e:
            import traceback; self._status(f"Tree error: {_e} {traceback.format_exc()[:200]}")
    ...
    def _find_first_populated(self, path):
        ...
        while queue:
            ...
            try:
                entries = list(os.scandir(_longpath(current)))
                for e in entries:
                    if e.is_file() and os.path.splitext(e.name)[1].lower() in exts:
    ...
    def _on_tree_folders_changed(self, parent_path=None):
        """Immediate structural refresh after folder create/delete/rename.

        FT.py owns tree state. ft_widgets performs the disk operation and calls
        this method; this method refreshes the visible tree rows so newly
        created folders appear immediately and deleted folders disappear.
        """
        try:
        ...
        try:
            parent = os.path.normpath(_ui_path(parent_path or getattr(self, 'current_folder', '') or self.mode_cfg.get('root', '')))
        except Exception:
            parent = self.mode_cfg.get('root', '')

        root = os.path.normpath(self.mode_cfg.get('root', ''))
        if not root or not os.path.isdir(_longpath(root)):
            return
        ...
        try:
            if parent and self.tree.exists(parent):
                # refresh_node: wipes children, calls _populate_level (disk scan),
                # then _fill_children_of on the new children via _on_node_open.
                self._tree_widget.refresh_node(parent)
                # Open the node so the new/remaining children are visible.
                self.tree.item(parent, open=True)
                # Refresh counts for the parent row itself.
                try: self._tree_widget._fill_node(parent)
                except Exception: pass
            else:
                self._populate_tree(root)
        except Exception:
            try: self._populate_tree(root)
            except Exception: pass

        try:
            pending = getattr(self, '_pending_folder_after_tree_change', '')
            if pending:
                self._pending_folder_after_tree_change = ''
                target = pending if self.tree.exists(pending) else parent
            else:
                selected = _ui_path(selected) if selected else ''
                target = selected if selected and self.tree.exists(selected) else parent
            if target and self.tree.exists(target):
                self.tree.selection_set(target)
                self.tree.focus(target)
                self.tree.see(target)
                # Do not depend on a TreeviewSelect event firing after a programmatic
                # structural refresh.  If the current folder was deleted, explicitly
                # load the real parent so future clicks are not poisoned by the
                # deleted path or a fake empty-folder state.
                if target and target == getattr(self, 'current_folder', None):
                    pass
                elif pending:
                    self._load_folder(target)
        except Exception:
            pass
        try: self._update_tree_viewing()
        except Exception: pass

    def _refresh_tree_stats_for_cache(self, folders=None):
        """Refresh visible tree thumbnail counts after cache generation.

        The normal full refresh handles visible nodes, but cache generation can
        update the selected folder and child folders while a modal result dialog
        is open.  This makes the affected folder rows refresh immediately and
        also refreshes their visible children/ancestors.
        """
        try:
            ...
            for folder in folders:
                cur = os.path.normpath(_ui_path(folder))
                while cur and cur not in seen:
                    seen.add(cur)
                    targets.append(cur)
                    if root and cur.lower() == root.lower():
                        break
            ...
            for folder in targets:
                if self.tree.exists(folder):
                    try:
                        self._tree_widget._fill_node(folder)
                    except Exception:
                        pass
                    for child in self.tree.get_children(folder):
                        if getattr(self._tree_widget, 'PLACEHOLDER', '__ph__') not in child:
                            try:
                                self._tree_widget._fill_node(child)
                            except Exception:
                                pass
            self.tree.update_idletasks()
        except Exception as e:
            print(f"_refresh_tree_stats_for_cache error: {e}")
    ...
    # ── Loading ────────────────────────────────────────────────────────────────
    def _show_no_files_in_folder(self, folder):
        """Show a stable empty-folder state without falling back to another folder."""
        self.current_folder = _ui_path(folder)
        try:
            self._all_files = []
            self._page_num = 0
            self._page_start = 0
            ...
            pass
        try:
            # Put the message in the thumbnail canvas/grid area.
            ...
            try:
                self._show_browse_prompt("No files in this folder")
            except Exception:
                pass
        try:
            self._status(f"No files in this folder: {folder}")
        except Exception:
            pass

    # ── Sort ──────────────────────────────────────────────────────────────────

    def _show_sort_menu(self, btn=None):
        """Show sort popup menu anchored to btn (defaults to toolbar sort button)."""
        show_file_sort_menu(
            btn or self.btn_sort,
            columns=[("Name", "name"), ("Date taken", "date_taken")],
            sort_column=self._sort_column,
            sort_reverse=self._sort_reverse,
            callback=self._set_sort,
        )

    def _set_sort(self, column: str, reverse: bool):
        self._sort_column  = column
        self._sort_reverse = reverse
        lbl = _sort_btn_label(column, reverse)
        for btn in (getattr(self, "btn_sort", None),
                    getattr(self, "_filelist_sort_btn", None)):
            try:
                if btn:
                    btn.config(text=lbl)
            except Exception:
    ...
    def _apply_sort(self):
        """Re-sort _all_files with current preference and redisplay from page 0."""
        ...
        try:
            ...
            self._all_files.sort(key=lambda p: os.path.basename(p).lower(),
                                 reverse=self._sort_reverse)
        self._page_num = 0
    ...
    def _load_folder(self, folder):
        ...
        exts = self._display_exts_for_folder(folder)
        import re as _re
        _date_pat = _re.compile(r'^\d{4}-\d{2}-\d{2}')

        # Default sort key (used when user sort = "name" and mode has special ordering)
        if self.mode == "photos" and not self._is_contact_sheets_folder(folder):
            def _default_sort_key(path):
                return os.path.basename(path).lower()
        else:
            # PDFs/Videos default: Scan* first, then dated-filename newest-first, then mtime desc
            def _default_sort_key(path):
        ...
        if self._recurse:
            ...
            files = sorted(all_paths, key=_default_sort_key)
        else:
            try:
                entries = [e for e in os.scandir(_longpath(folder))
                           if e.is_file() and os.path.splitext(e.name)[1].lower() in exts]
                files = sorted([_ui_path(e.path) for e in entries], key=_default_sort_key)
        ...
        if not files:
            ...
            if first and first != folder:
                ...
                self._load_folder(first)
                return
            # No images anywhere below — show centred message on grid and clear list/preview
            self._all_files = []
            self._populate_file_list([])
            self._clear_preview()
            self._clear_grid()
            self._show_no_images_message(folder)
            self._status(f"No {self._display_file_word_for_folder(folder)} files in this folder or subfolders")
            return
        # Do not scan the whole thumbnail DB here. That LIKE query can block
        # folder navigation on large projects/NAS paths. Page loading checks
        # cached thumbnails in one batch, so folder clicks stay responsive.
        self._no_thumb_msg = False
        self._all_files = files; self._page_num = 0
        self._populate_file_list(files)
        self._clear_preview()
        if not self._all_files:
            self._show_no_files_in_folder(folder)
            return
    ...
    def _populate_file_list(self, files):
        """Populate the FTmod file-list panel from the current folder/collection files."""
        lb = getattr(self, 'file_list', None)
        if not lb:
            return
        try:
            sig = tuple(files or [])
            if getattr(self, '_file_list_signature', None) == sig:
                return
            self._file_list_signature = sig
            self._syncing_file_list = True
            lb.delete(0, 'end')
            for path in files or []:
                lb.insert('end', _file_display_name(path))
            try:
                if hasattr(self, 'viewer'):
                    self.viewer.set_file_list(list(files or []), None)
            except Exception:
                pass
        except Exception as e:
            print(f"_populate_file_list error: {e}")
        finally:
            self._syncing_file_list = False

    def _sync_file_list_to_path(self, path):
        """Highlight the corresponding file-list row without relying on another event."""
        lb = getattr(self, 'file_list', None)
        if not lb or not path:
            return
        try:
            norm = os.path.normcase(os.path.normpath(path))
            self._syncing_file_list = True
            for i, f in enumerate(getattr(self, '_all_files', [])):
                if os.path.normcase(os.path.normpath(f)) == norm:
    ...
    def _on_file_list_mouse_down(self, event):
        ...
        try:
            ...
            if idx in lb.curselection():
                self.win.after(1, lambda i=idx: self._begin_file_list_rename(i))
                return "break"
        except Exception:
            pass
        return None

    def _begin_file_list_rename(self, idx):
        """Overlay an Entry on the file-list row and rename the filename stem only."""
        lb = getattr(self, 'file_list', None)
        if not lb:
            return
        try:
            ...
            if not bbox:
                return
            x, y, w, h = bbox
            entry = tk.Entry(lb, font=("Segoe UI", 9), relief="solid", bd=1)
            self._file_rename_entry = entry
            entry.insert(0, stem)
            entry.select_range(0, 'end')
            entry.place(x=x, y=y, width=max(60, w), height=h)
            entry.focus_set()
            ...
            def commit(event=None):
                ...
                if any(ch in new_stem for ch in r'\/:*?"<>|'):
                    messagebox.showerror("Rename file", "Filename cannot contain: \\ / : * ? \" < > |", parent=self.win)
                    return
                ...
                # FT/FTmod DB integration: preserve existing thumbnail blob under the new path,
                # then garbage-collect the old missing path from the folder.
                try:
                    blob = thumb_get(old_path) if self.use_stored_thumbs.get() else None
                    if blob:
                        thumb_put(new_path, blob)
                    thumb_gc(folder)
                except Exception as e:
                    print(f"Rename thumb DB update error: {e}")

                def same(p):
                    return os.path.normcase(os.path.normpath(p)) == os.path.normcase(os.path.normpath(old_path))
                def repl_list(seq):
                    return [new_path if same(p) else p for p in (seq or [])]
                self._all_files = repl_list(getattr(self, '_all_files', []))
                try:
                    self._unfiltered_files = repl_list(getattr(self, '_unfiltered_files', []))
                except Exception:
                    pass
                for attr in ('_selected', '_placed_set', '_right_sel', '_culled', '_edited', '_unreadable'):
                    try:
                        st = getattr(self, attr, None)
                        if isinstance(st, set) and old_path in st:
                            st.discard(old_path); st.add(new_path)
                    except Exception:
                        pass
                for attr in ('tagged_at', '_culled_at', '_unreadable_reason', '_rot_delta'):
                    try:
                ...
                try:
                    if old_path in self.tagged:
                        self.tagged.discard(old_path); self.tagged.add(new_path)
                    self.tagged_order = repl_list(getattr(self, 'tagged_order', []))
                except Exception:
                    pass
                self._file_list_signature = None
                self._populate_file_list(self._all_files)
                self._sync_file_list_to_path(new_path)
                self._show_preview_for_path(new_path)
                try:
                    self._show_page()
                except Exception:
                    pass
                try:
                    if hasattr(self, 'tree_widget') and hasattr(self.tree_widget, 'refresh_after_file_ops'):
                        self.tree_widget.refresh_after_file_ops([folder])
                    elif hasattr(self, 'folder_tree') and hasattr(self.folder_tree, 'refresh_after_file_ops'):
                        self.folder_tree.refresh_after_file_ops([folder])
                except Exception:
                    pass
    ...
    def _on_file_list_select(self, event=None):
        """File-list click: show preview and scroll the thumbnail into view.

        It does NOT alter the FT operation selection set. Selection for copy/move/delete
        is controlled from thumbnails with Ctrl-click and Shift-click, matching FTView.
        """
        lb = getattr(self, 'file_list', None)
        if not lb or getattr(self, '_syncing_file_list', False):
            return
        try:
            sel = lb.curselection()
            if not sel:
            ...
            print(f"_on_file_list_select error: {e}")

    def _zoom_current_filelist_item(self):
        try:
            lb = getattr(self, 'file_list', None)
            if not lb:
                return
            ...
            if not sel:
                return
            idx = int(sel[0])
            if 0 <= idx < len(self._all_files):
                self._zoom_and_focus(self._all_files[idx])
        except Exception:
            pass

    def _scroll_thumb_to_index(self, idx):
        """Move to the page containing idx and try to keep the thumb visible."""
        try:
            page_size = max(1, getattr(self, '_page_size', 1))
            target_page = idx // page_size
            if target_page != getattr(self, '_page_num', 0):
                self._page_num = target_page
                self._show_page()
            else:
                pos = idx - getattr(self, '_page_start', target_page * page_size)
                row = pos // max(1, getattr(self, '_cols', 1))
                cell_h = max(1, getattr(self, '_disp_size', THUMB_SIZE))
                self.canvas.yview_moveto(max(0.0, min(1.0, (row * cell_h) / max(1, self.canvas.bbox('all')[3] if self.canvas.bbox('all') else 1))))
        except Exception:
            pass

    def _clear_preview(self):
        """Clear the shared FTView viewer panel."""
        self._preview_path = None
        self._preview_photo = None
        try:
            if hasattr(self, 'viewer'):
                self.viewer.set_file_list(getattr(self, '_all_files', []), None)
                if not getattr(self, '_all_files', []):
                    self.viewer.show_message('No files in this folder')
                else:
                    self.viewer.show_message('No file selected')
        except Exception:
            pass

    def _show_preview_for_path(self, path):
        """Show a large preview using the shared FTView ViewerPanel."""
        self._preview_path = path
        self._sync_file_list_to_path(path)
        try:
            files = list(getattr(self, '_all_files', []) or [])
            idx = files.index(path) if path in files else None
            if hasattr(self, 'viewer'):
                self.viewer.set_file_list(files, idx)
        except Exception as e:
            print(f"_show_preview_for_path error: {e}")

    def _refresh_preview_image(self):
        """Compatibility wrapper: reload current file in the shared viewer."""
        try:
            if hasattr(self, 'viewer') and getattr(self, '_preview_path', None):
                self._show_preview_for_path(self._preview_path)
        except Exception:
            pass

    def _viewer_select_index(self, idx):
        """Viewer chevrons selected a different file: sync file list and thumbs."""
        try:
            files = list(getattr(self, '_all_files', []) or [])
            if idx < 0 or idx >= len(files):
                return
            ...
            self._scroll_thumb_to_index(idx)
            if hasattr(self, 'viewer'):
                self.viewer.show_index(idx)
        except Exception as e:
            print(f"_viewer_select_index error: {e}")

    def _on_viewer_file_changed(self, path):
        """Shared viewer rotated/changed a file: update FT thumbnail DB/cell."""
        try:
            if path:
                self._make_thumb(path)
                self._refresh_thumb_cell(path)
                self._refresh_cell_size(path)
                self._show_preview_for_path(path)
        except Exception as e:
            print(f"_on_viewer_file_changed error: {e}")

    def _compute_grid_dims(self):
        """Compute thumbnail grid geometry using the same shared FTView rules.

        FTmod must not have its own thumbnail sizing maths.  The only input
        difference from FTView is the available thumbnail panel size.
        Rules are owned by ft_thumb_layout.calculate_thumb_layout:
            cell_width = (thumb_area_width - gap*(cols-1) - boundary*2) / cols
            cell_height = cell_width / 0.85
            image_size = cell_width - 10
        """
        ...
        if ch < 10:
            ch = max(400, self.win.winfo_height() - 120)

        scrollbar_w = 17
        avail_w = max(100, int(cw) - scrollbar_w)
        avail_h = max(100, int(ch))

        # IMPORTANT: 1-panel and 2-panel modes must use the same shared
        # FTView thumbnail geometry.  The only difference is the actual canvas
        # width/height available after the sash/divider position is applied.
        # Do not fall back to the old compact 120px 2-panel sizing here.
        requested_cols = int(getattr(self, "_user_cols", 0) or 0)
        if requested_cols <= 0:
            # Auto mode starts from the current/base image size and derives
            # a visible column count from the actual panel width.
            base_img = max(80, int(getattr(self, "_disp_size", THUMB_SIZE) or THUMB_SIZE))
            base_cell_w = base_img + 10
            requested_cols = max(1, int((avail_w - 6) // max(1, base_cell_w + 6)))
            if canvas_ready:
                self._user_cols = requested_cols

        cols = max(1, requested_cols)
        layout = calculate_thumb_layout(
            panel_width=avail_w,
            panel_height=avail_h,
            item_count=max(1, len(getattr(self, "_all_files", []) or [])),
            columns=cols,
            gap=6,
            boundary_gap=3,
        ...
        # Visible rows for paging.  Partial bottom row is allowed visually,
        # but page size uses complete rows so the user never gets a clipped
        # final thumbnail row as the only way to reach a file.
        row_pitch = layout.cell_h_px + layout.gap
        visible_rows = max(1, int((avail_h - (2 * layout.boundary_gap) + layout.gap) // max(1, row_pitch)))

        self._thumb_layout = layout
        self._disp_size = int(layout.image_w_px)
        self._cols = int(layout.columns)
        self._page_size = self._cols * visible_rows

        # Print only when geometry changes, so diagnostics are useful but not noisy.
        sig = (getattr(self, "_panel_mode", "1"), layout.panel_width, layout.panel_height, layout.columns,
               layout.cell_w_px, layout.cell_h_px, layout.image_w_px, visible_rows)
        if getattr(self, "_last_thumb_layout_sig", None) != sig:
            self._last_thumb_layout_sig = sig
            print_thumb_layout(layout, prefix="FTmod shared thumb layout")

        self._updating_spinners = True
        try:
            self._cols_var.set(str(self._cols))
        except Exception:
            pass
        finally:
            self._updating_spinners = False
        return self._cols, self._disp_size

    def _show_page(self):
        """Show current page of thumbnails."""
        self._populate_file_list(getattr(self, '_all_files', []))
        self._loading = True
    ...
    def _load_page_thread(self, page, start_idx, gen):
        ...
        use_stored = True
        try:
            use_stored = bool(self.use_stored_thumbs.get())
        except Exception:
            use_stored = True

        # Display render size.  Stored DB thumbnails remain fixed at 250x250,
        # but screen-resolution mode deliberately renders larger source thumbs.
        sz = self._disp_size if use_stored else self._display_thumb_render_size(self._disp_size)

        # Pre-cache file sizes and GPS coords so badges appear on first render
        for orig in page:
            _file_size_info_cached(orig)
            if os.path.splitext(orig)[1].lower() in PHOTO_EXTS:
        ...
        jpeg_map = thumb_get_many(page) if use_stored else {}
        if not use_stored:
            jpeg_map = {}

        force_direct_doc_thumbs = (getattr(self, 'mode', '') == 'pdfs' and not use_stored)

        # Generate any missing thumbnails into memory
        new_items = []
        _page_idx = {orig: i for i, orig in enumerate(page)}   # for progressive per-cell renders
        missing = [p for p in page if jpeg_map.get(p) is None]
        if use_stored and missing and self._auto_cache:
            self.win.after(0, self._status, f"Generating {len(missing)} thumbnail{'s' if len(missing)!=1 else ''}...")
        for orig in page:
            if self._load_gen != gen: return
            if use_stored and jpeg_map.get(orig) is None and (self._auto_cache or mode == "videos"):
                jpeg_bytes = None
                try:
                    ext = os.path.splitext(orig)[1].lower()
                    if ext in VIDEO_EXTS and HAVE_VIDEO_THUMB:
                        img, _vt_ok, _vt_err = _make_video_thumb(orig, 0.1, THUMB_STORE_SIZE, _longpath)
                        if img is not None:
                            buf = _io.BytesIO()
                            img.save(buf, "JPEG", quality=82)
                            jpeg_bytes = buf.getvalue()
                            # Commit immediately so FTVideo (launched concurrently) finds it in DB
                            try:
                                thumb_put_many([(orig, jpeg_bytes)])
                            except Exception:
                                pass
                            # Progressive: show this frame immediately without waiting for rest
                            _disp = _scale_to_fit(img, sz)
                            self.win.after(0, self._render_one, orig,
                                           start_idx + _page_idx.get(orig, 0),
                                           _disp, False, False, gen)
                    elif ext in VIDEO_EXTS:
                        pass  # HAVE_VIDEO_THUMB=False; no fallback for videos
                    elif ext in PDF_EXTS and HAVE_FITZ:
                        doc = fitz.open(_longpath(orig))
                        pg0 = doc[0]
                        scale = THUMB_STORE_SIZE / max(pg0.rect.width, pg0.rect.height)
                        mat = fitz.Matrix(scale, scale)
                        pix = pg0.get_pixmap(matrix=mat, alpha=False)
                        ...
                        ext = os.path.splitext(orig)[1].lower()
                        if ext in DOCX_EXTS:
                            # DOCX thumbnails use the shared ft_viewer Office-preview path:
                            # DOCX -> cached preview PDF -> first-page thumbnail.
                            img, ok, _err = make_preview_thumbnail(
                                orig,
                                sz,
                                longpath_func=_longpath,
                            )
                            ...
                            jpeg_bytes = _thumb_make_jpeg_from_path(
                                orig,
                                THUMB_STORE_SIZE,
                                longpath_func=_longpath,
                                quality=82,
                            )
                except Exception as _thumb_ex:
                    print(f"FTmod thumb exception ({os.path.basename(orig)}): {_thumb_ex}")
                if jpeg_bytes:
        ...
        # Write remaining new thumbnails to blob (videos already saved individually above)
        non_video_items = [(p, j) for p, j in new_items
                           if os.path.splitext(p)[1].lower() not in VIDEO_EXTS]
        if use_stored and non_video_items:
            self.win.after(0, self._status, f"Saving {len(non_video_items)} thumbnail{'s' if len(non_video_items)!=1 else ''} to blob...")
            try:
                thumb_put_many(non_video_items)
            except Exception as _e:
        ...
        for i, orig in enumerate(page):
            if self._load_gen != gen: return
            if force_direct_doc_thumbs:
                img = self._render_document_thumb_direct(orig, sz)
                renders.append((orig, start_idx + i, img, False, False))
                continue
            ...
            if jpeg:
                try:
                    ...
                    try:
                        self._unreadable.discard(orig)
                        self._unreadable_reason.pop(orig, None)
                    except Exception:
                        pass
                    ...
                    pass
            # No cached thumbnail — open directly from disk/preview.
            # Always try: auto_cache=off just means we don't save the result.
            img2 = None
            ext = os.path.splitext(orig)[1].lower()

            # Video files cannot be opened by PIL — skip straight to placeholder.
            # Do NOT mark as unreadable; the 🎬 icon in _add_cell covers it.
            if ext in VIDEO_EXTS:
                from PIL import Image as _PImgV
                ph = _PImgV.new('RGB', (sz, sz), (30, 30, 30))
                renders.append((orig, start_idx + i, ph, False, False))
                continue

            # DOCX must not be opened with PIL as an image.  Route it through
            # the same FTViewDocs path: DOCX -> cached preview PDF -> first page.
            if ext in DOCX_EXTS:
                try:
                    img2, ok, _err = make_preview_thumbnail(
                        orig,
                        sz,
                        longpath_func=_longpath,
                    )
                ...
                for try_path in (_longpath(orig), orig):
                    try:
                        if ext in PDF_EXTS and HAVE_FITZ:
                            ...
                            page0 = doc[0]
                            _pdf_scale = sz / max(page0.rect.width, page0.rect.height)
                            ...
                            img2.thumbnail((sz, sz), Image.LANCZOS)
            ...
            try:
                ...
                # Do not save grey placeholders for DOCX.  A DOCX conversion
                # may fail temporarily while Word/LibreOffice is busy; caching
                # a grey placeholder poisons the thumbnail DB until manual clear.
                if os.path.splitext(orig)[1].lower() not in DOCX_EXTS:
                    buf3 = _io.BytesIO()
        ...
        def _render_all(renders=renders, gen=gen, no_cache=no_cache, new_items=new_items):
            ...
            def _render_chunk():
                ...
                # Refresh tree thumb counts whenever thumbnails were loaded or generated
                if use_stored and (new_items or jpeg_map):
    ...
    def _add_cell_from_img(self, orig, idx, img, from_cache, gen, ghost=False):
        try:
            layout = getattr(self, '_thumb_layout', None)
            sz = int(getattr(layout, 'image_w_px', None) or self._disp_size)
            photo = _photo_cache_get(orig, sz, 'stored')
            if photo is None:
                # Shared FTView rule: image area is always square.
                img.thumbnail((sz, sz), Image.LANCZOS)
                photo = ImageTk.PhotoImage(img)
                _photo_cache_put(orig, sz, photo, 'stored')
        except Exception:
            return
    ...
    def _add_cell_2panel_left(self, orig, idx, photo, gen):
        ...
        _ext_lower = os.path.splitext(orig)[1].lower()
        _is_photo = _ext_lower in PHOTO_EXTS
        _is_video = _ext_lower in VIDEO_EXTS
        _, cat, disp_s = _file_size_info_cached(orig)
        size_info = f"{cat}  {disp_s}"
        if os.path.splitext(orig)[1].lower() in (PDF_EXTS | DOCX_EXTS):
        ...
        icon  = "📷 " if _is_photo else ("🎬 " if _is_video else "📄 ")
        ...
        # Sequence badge
        seq_num = idx + 1
        cv.create_rectangle(IX, IY, IX + 39, IY + 24, fill="#000000", outline="")
        cv.create_text(IX + 4, IY + 12, anchor="w", text=str(seq_num),
                       fill="white", font=("Segoe UI", 12, "bold"))

        # Scissors badge — top-right, double size of seq badge, launches FTEditI
        if _is_photo:
            _sx0 = IX + sz - 39
            cv.create_rectangle(_sx0, IY, IX + sz, IY + 24, fill="#000000", outline="")
            cv.create_text(_sx0 + 19, IY + 12, anchor="center", text="✂",
                           fill="white", font=("Segoe UI", 12, "bold"),
                           tags="scissors_badge")

        # MOVED watermark on the left-panel copy after it has been moved to the right panel.
        ...
        def _click(e, o=orig, c=cv,
            ...
            if os.path.splitext(o)[1].lower() in PHOTO_EXTS:
                _sx0 = _IX + _sz - 39
                if _sx0 <= xi <= _IX + _sz and _IY <= yi <= _IY + 24:
            ...
            # Image area: FTView-style behaviour.
            #   Click       = show image + jump file list only
            #   Ctrl-click  = select/deselect one thumb
            #   Shift-click = apply previous select/deselect action to range
            if yi <= _IY + _IMG_H:
                self._show_preview_for_path(o)
                ctrl = (e.state & 0x4) != 0
                shift = (e.state & 0x1) != 0 and not ctrl
                ...
                if ctrl:
                    if o in self._selected:
                        ...
                        self._last_select_action = "deselect"
                    else:
                        self._selected.add(o)
                        self._last_select_action = "select"
                    self._last_click_idx = cur_idx
                    self._repaint_selection(changed_only={o})
                    self._update_sel_bar()
                elif shift:
                    ...
                    action = getattr(self, '_last_select_action', 'select')
                    changed = set()
                    ...
                    # Plain click is preview/navigation only. Do not alter operation selection.
                    self._last_click_idx = cur_idx

        cv.bind("<Button-1>", _click)
        cv.bind("<Button-3>", lambda e, o=orig: self._show_thumb_context_menu(e, o))
    ...
    def _add_cell(self, orig, idx, photo, from_cache, gen, ghost=False):
        ...
        if getattr(self, "_panel_mode", "1") == "2" and orig in getattr(self, "_placed_set", set()):
            selected = False
        bd_col, mid_col = GHOST_BD if ghost else UNTAGGED_BD, GHOST_BD if ghost else UNTAGGED_BD

        layout = getattr(self, '_thumb_layout', None)
        sz      = int(getattr(layout, 'image_w_px', None) or self._disp_size)
        IMG_H   = sz  # Shared FTView rule: image area is square
        CELL_W  = int(getattr(layout, 'cell_w_px', sz + 10))
        CELL_H  = int(getattr(layout, 'cell_h_px', max(sz + 58, int((sz + 10) / 0.85))))
        BD      = 3          # outer border width; ornaments retained
        PAD     = 3          # boundary half-gap / background inset
        IX      = int(getattr(layout, 'image_x_px', 5))
        IY      = int(getattr(layout, 'image_y_px', 5))
        ...
        fname    = os.path.basename(orig)
        icon     = ("🗑 " if ghost else
                    ("⚡ " if from_cache else ("📷 " if _is_photo else ("🎬 " if _is_video else "📄 "))))
        ...
        if ghost:
            ...
            has_gps = _is_photo and bool(_get_gps_coords(orig))
            if has_gps:
                gx = IX + sz // 2
            ...
            def _cv_click(e, o=orig, c=cv,
                          ...
                          _is_photo=_is_photo, _is_video=_is_video):
                xi, yi = e.x, e.y
                # Scissors badge — top-right, launches FTEditI
                if _is_photo and not ghost:
                    _sx0 = _IX + _sz - 39
                ...
                if _CTL_Y <= yi <= _CTL_Y + _CTL_H:
                    ...
                    if _is_photo:
                        ...
                        # PDFs/Videos: left half = Mark, right half = Zoom
                        if xi <= _IX + _rm:
                            self._toggle_cull_canvas(o, c); return
                        else:
                            self._zoom_and_focus(o); return
    ...
    def _show_no_images_message(self, folder):
        ...
        word = self._display_file_word_for_folder(folder)
    ...
    def _show_welcome_screen(self):
        ...
        tk.Label(card, text="FTmod",
                 bg=_card_bg, fg=_card_fg,
                 font=("Segoe UI", 52, "bold"),
                 padx=60).pack(pady=(30, 0))

        tk.Label(card,
                 text="📷  Photos    +    📄  Documents    —    Tag  ·  Browse  ·  Export",
                 bg=_card_bg, fg=_card_fg,
                 font=("Segoe UI", 13),
    ...
    def _show_browse_prompt(self):
        ...
        mode_word = "Photos" if self.mode == "photos" else ("Videos" if self.mode == "videos" else "Documents")
    ...
    def _ftediti_script(self):
        ...
        return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "helpers", "FTImgedit.py")
    ...
    def _ftmapimg_script(self):
        return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "helpers", "FTMap.py")
    ...
    def _ftvideo_script(self):
        return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "helpers", "FTVideo.py")
    ...
    def _launch_ftvideo_from_selection(self):
        ...
        if not video_files:
            messagebox.showinfo("No videos",
                "No video files found in the current selection.\n\n"
                "Select video files first, or switch to Videos mode.",
                parent=self.win)
            return
        self._launch_ftvideo(video_files)
    ...
    def _launch_ftvideo(self, files):
        ...
        if not os.path.isfile(script):
            messagebox.showwarning("FTVideo not found",
                f"FTVideo.py not found:\n{script}", parent=self.win)
            return
        proc = getattr(self, '_ftvideo_proc', None)
        if proc is None or proc.poll() is not None:
            try:
                self._ftvideo_proc = subprocess.Popen(
                    [sys.executable, script, "--embedded"])
                self.win.after(1000, lambda: self._send_to_ftvideo(files))
            except Exception as e:
                messagebox.showerror("Cannot launch FTVideo", str(e), parent=self.win)
        else:
            self._send_to_ftvideo(files)

    def _send_to_ftvideo(self, files):
        """Write IPC request for FTVideo — sends file list."""
        self._ftvideo_seq = getattr(self, '_ftvideo_seq', 0) + 1
        try:
            ipc_dir  = self._ftediti_ipc_dir()
            req_path = os.path.join(ipc_dir, "FTVideo_request.csv")
            with open(req_path, "w", encoding="utf-8") as f:
                f.write(f"SEQ,{self._ftvideo_seq}\n")
                for fp in files:
                    f.write(f"{fp}\n")
        except Exception as e:
            print(f"FTDBX: could not write FTVideo request: {e}")

    # ── FTFiler integration ───────────────────────────────────────────────────

    def _ftfiler_script(self):
        return os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "helpers", "FTFiler.py")
    ...
    def _handle_rename(self, old_path, new_path):
        ...
        if _db() is not None:
            try:
                ft_db.rekey_file_paths(_db(), old_path, new_path)
            except Exception as e:
                print(f"_handle_rename DB error: {e}")


    def _remove_thumbs_current(self, folder=None):
        """Remove cached thumbnails for the selected/current folder only."""
        folder = _ui_path(folder or getattr(self, "current_folder", ""))
        if not folder:
            return
        if not messagebox.askyesno(
            "Remove Thumbnails",
            f"Remove cached thumbnails for this folder?\n\n{folder}\n\n"
            "This will NOT delete or modify any files.",
            parent=self.win
        ):
            return

        removed = ft_db.delete_thumbs_for_folder(_db(), folder)

        # Clear in-memory PhotoImage cache for this folder.
        try:
        ...
        # Refresh visible tree counts.
        try:
            self._tree_widget.refresh_stats()
        except Exception:
            pass

        # Reload current folder if this is the one being viewed.
        try:
            if _ui_path(getattr(self, "current_folder", "")) == folder:
                self._load_folder(folder)
        except Exception:
            pass

        try:
            self._status(f"Removed {removed} cached thumbnail row{'s' if removed != 1 else ''}")
        except Exception:
            pass

    def _on_tree_right_click(self, event):
        """Tree right-click menu with FileTagger thumbnail-cache action added."""
        item = self.tree.identify_row(event.y)
        if not item:
            return "break"
        placeholder = getattr(self._tree_widget, "PLACEHOLDER", "__ph__")
        ...
        try:
            menu = self._tree_widget._make_context_menu(item)
        except Exception:
    ...
    def _remove_thumbs_for_folder_no_prompt(self, folder):
        """Remove cached thumbnails for a folder/subtree without prompting.

        Used by destructive folder-delete operations where the user has already
        confirmed deletion.
        """
        folder = _ui_path(folder)
        if not folder:
    ...
    def _remove_thumbs_for_folder(self, folder):
        """Remove cached thumbnails for a folder selected in the tree."""
        folder = _ui_path(folder)
        if not folder:
            return
        if not messagebox.askyesno(
        ...
        # Clear in-memory thumb image cache for this folder.
        try:
            prefix = folder.rstrip("\\/") + os.sep
            for key in list(_photo_cache.keys()):
                try:
        ...
        # Reload current folder if applicable.
        try:
            if _ui_path(getattr(self, "current_folder", "")) == folder:
                self._load_folder(folder)
        except Exception:
            ...
            if action == "delete_all":
                if _db() is not None:
                    try:
                        ...
                        ft_db.remove_collection_items(_db(), cull_set)
                    except Exception as e:
    ...
    def _context_selected_files_for(self, path):
        """Return the right-click operation set, selecting the clicked item if needed."""
        if path and os.path.isfile(path) and path not in self._selected:
            self._selected.add(path)
            self._last_select_action = "select"
            try:
                self._last_click_idx = self._all_files.index(path)
            except Exception:
                pass
            try:
                ...
                pass
        return [p for p in self._all_files if p in self._selected and os.path.isfile(p)]

    def _show_thumb_context_menu(self, event, path):
        """Right-click operations menu for FTmod thumbnails.

        Matches FTView's selected-item operation menu, with FTmod-specific PDF
        Edit support that opens the grayscale/B/W dialog.
        """
        try:
            if not path or not os.path.isfile(path):
                return "break"
            files = self._context_selected_files_for(path)
            if not files:
                files = [path]
            ext = os.path.splitext(path)[1].lower()
            menu = tk.Menu(self.win, tearoff=0)
            if ext == ".pdf":
                pdf_targets = [p for p in files if os.path.splitext(p)[1].lower() == ".pdf"]
                if path not in pdf_targets:
                    pdf_targets = [path]
                menu.add_command(label="Edit", command=lambda f=list(pdf_targets): self._show_pdf_edit_dialog(f))
                menu.add_command(label="Add to PDF Merge Queue", command=lambda f=list(pdf_targets): self._add_to_pdf_merge_queue(f))
                menu.add_command(label="Open PDF Merge Queue", command=self._open_pdf_merge_queue)
                menu.add_command(label="Merge PDF Merge Queue", command=self._merge_pdf_merge_queue)
                menu.add_separator()
            elif ext in (".jpg", ".jpeg"):
                menu.add_command(label="Edit JPG in FTImgedit", command=lambda p=path: self._launch_ftediti(p))
                menu.add_separator()
            else:
                menu.add_command(label="Edit", state="disabled")
                menu.add_separator()

            n = len(files)
            menu.add_command(label=f"Copy selected ({n})", command=lambda f=list(files): self._context_copy_selected(f))
            menu.add_command(label=f"Move selected ({n})", command=lambda f=list(files): self._context_move_selected(f))
            # Contact sheet is only useful for image/PDF files, and FTmod already
            # has the shared contact-sheet dialog behind this method.
            menu.add_command(label=f"Contact sheet selected ({n})", command=self._ops_contact_sheet)
            menu.add_command(label=f"Print selected ({n})", command=lambda f=list(files): self._print_selected_files(f))
            menu.add_separator()
            menu.add_command(label=f"Delete selected ({n})", command=lambda f=list(files): self._context_delete_selected(f))
            try:
                menu.tk_popup(event.x_root, event.y_root)
            finally:
                try:
            ...
            messagebox.showerror("Thumbnail operations", str(e), parent=self.win)
            return "break"


    def _print_selected_files(self, files):
        """Print routing for selected files.

        Documents:
            PDF/DOCX selections are converted/combined into one temporary PDF,
            then opened in the system PDF viewer so the user can print once.

        Images:
            JPG/JPEG selections use the existing contact-sheet workflow.
        """
        try:
            files = [os.path.normpath(f) for f in (files or []) if f and os.path.isfile(_longpath(f))]
            if not files:
                messagebox.showinfo("Print selected", "No selected files to print.", parent=self.win)
                return
            ...
            if image_files and not doc_files and not other_files:
                # Images are best printed via the contact-sheet dialog.
                old_selected = set(getattr(self, "_selected", set()))
                try:
                    self._selected = set(image_files)
                    self._ops_contact_sheet()
                finally:
            ...
            if doc_files and not image_files and not other_files:
                result = ft_print.print_documents_as_combined_pdf(
                    doc_files,
                    parent=self.win,
                    title="FTMod selected documents"
                )
                if result.get("ok"):
                    messagebox.showinfo(
                        "Print selected",
                        "A temporary combined PDF has been opened in the system PDF viewer.\n\n"
                        "Print it from that viewer.",
                        parent=self.win
                    )
                else:
                    messagebox.showerror("Print selected", result.get("message", "Print preparation failed."), parent=self.win)
                return

            # Mixed image + document selection: keep behaviour explicit and safe.
            msg = (
                "The selection contains both images and documents.\n\n"
                "Images are printed through Contact Sheet.\n"
                "PDF/DOCX documents are combined into one temporary PDF.\n\n"
                "Run the image and document print actions separately."
            )
            messagebox.showwarning("Print selected", msg, parent=self.win)
        except Exception as e:
            messagebox.showerror("Print selected", str(e), parent=self.win)


    def _ordered_current_pdf_selection(self, files=None):
        """Return selected/placed PDF files in visible order, with duplicates removed."""
        if files is None:
            if getattr(self, "_placed", None):
                files = list(self._placed)
            elif getattr(self, "_selected", None):
                files = [p for p in getattr(self, "_all_files", []) if p in self._selected]
            else:
                files = []
        seen = set()
        result = []
        for p in files:
            try:
                ...
                if p in seen:
                    continue
                if os.path.splitext(p)[1].lower() == ".pdf" and os.path.isfile(_longpath(p)):
                    result.append(p)
                    seen.add(p)
            except Exception:
                pass
    ...
    def _ensure_pdf_merge_queue(self):
        root = self.mode_cfg['root']
        if PDF_MERGE_QUEUE not in _list_collections(root):
            _write_collection(PDF_MERGE_QUEUE, root, set(), {})
            self._refresh_collection_list()
        return PDF_MERGE_QUEUE

    def _add_to_pdf_merge_queue(self, files=None):
        """Append selected PDFs to the persistent PDF Merge Queue."""
        pdfs = self._ordered_current_pdf_selection(files)
        if not pdfs:
            messagebox.showinfo("PDF Merge Queue", "No PDF files are selected.", parent=self.win)
            return
        root = self.mode_cfg['root']
        self._ensure_pdf_merge_queue()
        added = _append_collection_items(PDF_MERGE_QUEUE, root, pdfs)
        self._refresh_collection_list()
        self._status(f"✓  {added} PDF file{'s' if added != 1 else ''} appended to {PDF_MERGE_QUEUE}")
        if added == 0:
            messagebox.showinfo("PDF Merge Queue", "Those PDF files are already in the PDF Merge Queue.", parent=self.win)

    def _open_pdf_merge_queue(self):
        """Open the reserved persistent PDF Merge Queue collection."""
        self._ensure_pdf_merge_queue()
        self._switch_collection(PDF_MERGE_QUEUE, confirm=True)
        data = _read_collection(PDF_MERGE_QUEUE, self.mode_cfg['root'])
        if not data:
            self._in_tagged_view = True
            self._all_files = []
            self._page_num = 0
            self.lbl_folder.config(text=f"★  {PDF_MERGE_QUEUE} — empty")
            self._update_view_status(f"★  {PDF_MERGE_QUEUE} — empty")
            self._show_page()
            messagebox.showinfo("PDF Merge Queue", "The PDF Merge Queue is empty.", parent=self.win)
            return
        self._show_collection_view()

    def _clear_pdf_merge_queue(self):
        """Clear the reserved PDF Merge Queue without deleting any source files."""
        self._ensure_pdf_merge_queue()
        data = _read_collection(PDF_MERGE_QUEUE, self.mode_cfg['root'])
        if not data:
            messagebox.showinfo("PDF Merge Queue", "The PDF Merge Queue is already empty.", parent=self.win)
            return
        if not messagebox.askyesno(
            "Clear PDF Merge Queue",
            f"Remove {len(data)} file{'s' if len(data) != 1 else ''} from the PDF Merge Queue?\n\nSource PDF files will not be deleted.",
            parent=self.win,
        ):
            return
        _write_collection(PDF_MERGE_QUEUE, self.mode_cfg['root'], set(), {}, [])
        if self.collection == PDF_MERGE_QUEUE:
            self.tagged.clear(); self.tagged_at.clear(); self.tagged_order = []
            self._all_files = []
            self._show_page()
            self._update_coll_info()
        self._refresh_collection_list()
        self._status("✓  PDF Merge Queue cleared")

    def _merge_pdf_merge_queue(self):
        """Merge PDFs from right-panel order if present, otherwise saved queue order."""
        self._ensure_pdf_merge_queue()
        # If the user is actively arranging PDFs in the right panel, honour that visual order.
        source = "right panel order" if getattr(self, "_placed", None) else "saved queue order"
        if getattr(self, "_placed", None):
            pdfs = self._ordered_current_pdf_selection(list(self._placed))
        else:
            if self.collection == PDF_MERGE_QUEUE:
                try:
                    self._save_current_collection()
                    ...
                    pass
            data = _read_collection(PDF_MERGE_QUEUE, self.mode_cfg['root'])
            pdfs = self._ordered_current_pdf_selection(list(data.keys()))

        if len(pdfs) < 2:
            messagebox.showinfo("Merge PDFs", "The PDF Merge Queue needs at least two valid PDF files.", parent=self.win)
            return

        try:
            from tkinter import filedialog as _fd
            from datetime import datetime, timezone, timedelta
            init_dir = os.path.dirname(pdfs[0]) or self.current_folder or os.getcwd()
            default_name = datetime.now(timezone(timedelta(hours=10))).strftime("Merged_%Y-%m-%d_%H%M.pdf")
            output_pdf = _fd.asksaveasfilename(
                parent=self.win,
                title=f"Save merged PDF ({source})",
                initialdir=init_dir,
        ...
        try:
            result = ft_pdf_ops.merge_pdfs(pdfs, output_pdf)
        except Exception as e:
            messagebox.showerror("Merge PDFs", str(e), parent=self.win)
            return

        try:
            if os.path.dirname(os.path.normpath(result.output_path)) == os.path.normpath(getattr(self, "current_folder", "")):
                if result.output_path not in self._all_files:
                    self._all_files.append(result.output_path)
                    self._all_files.sort(key=str.lower)
                    self._show_page()
        except Exception:
            pass
        ...
        messagebox.showinfo(
            "Merge PDFs",
            f"Merged {result.input_files} PDF files into:\n{result.output_path}\n\nPages: {result.pages}\nOrder used: {source}.",
            parent=self.win,
        )
        self._status(f"✓  Merged {result.input_files} PDFs → {os.path.basename(result.output_path)}")

    def _show_pdf_edit_dialog(self, paths):
        """Show PDF edit choices: grayscale or true B/W copies."""
        paths = [os.path.normpath(p) for p in (paths or [])
                 if p and os.path.splitext(p)[1].lower() == ".pdf" and os.path.isfile(_longpath(p))]
        if not paths:
            messagebox.showinfo("Edit PDF", "No PDF files are selected.", parent=self.win)
            return
        dlg = tk.Toplevel(self.win)
        dlg.title("Edit PDF")
        dlg.transient(self.win)
        ...
        dlg.configure(bg=BG3)
        try:
            self._centre_window(dlg, 380, 150)
        except Exception:
            pass
        what = f"{len(paths)} selected PDF files" if len(paths) != 1 else os.path.basename(paths[0])
        tk.Label(dlg, text=f"Create edited copy for:\n{what}",
                 bg=BG3, fg=TEXT_BRIGHT, font=("Segoe UI", 10, "bold"),
                 justify="left", padx=14, pady=12).pack(fill="x")
        ...
        tk.Button(bf, text="Grayscale", command=lambda: run("grayscale"),
                  bg="#1a3a6a", fg="white", font=("Segoe UI", 9, "bold"),
                  relief="flat", padx=12, pady=5).pack(side="left", padx=(0, 8))
        tk.Button(bf, text="B/W", command=lambda: run("bw"),
                  bg="#1a3a6a", fg="white", font=("Segoe UI", 9, "bold"),
                  relief="flat", padx=12, pady=5).pack(side="left", padx=(0, 8))
        tk.Button(bf, text="Split into pages", command=lambda: run("split"),
                  bg="#1a6655", fg="white", font=("Segoe UI", 9, "bold"),
                  relief="flat", padx=12, pady=5).pack(side="left", padx=(0, 8))
        tk.Button(bf, text="Cancel", command=dlg.destroy,
                  bg="#555555", fg="white", relief="flat", padx=12, pady=5).pack(side="right")

    def _context_choose_destination(self, title):
        """Choose a Copy/Move destination using the shared FTView-style dialog.

        The dialog intentionally does not grab the application. While it is open,
        clicking a folder in FTmod's main folder tree updates the destination
        field instead of navigating away. This matches FTView and keeps folder
        selection behaviour shared in spirit between the two apps.
        """
        dlg = tk.Toplevel(self.win)
        dlg.title(title)
        dlg.transient(self.win)
        ...
        root = _ui_path((self.mode_cfg or {}).get("root", "") or getattr(self, "current_folder", "") or os.getcwd())
        init = _ui_path(getattr(self, "current_folder", "") or root)
        folder_var = tk.StringVar(value=init)
        self._destination_dialog_active = True
        self._destination_folder_var = folder_var

        tk.Label(dlg, text=title, bg=BG3, fg=TEXT_BRIGHT,
                 font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=10, pady=(10, 4))

        row = tk.Frame(dlg, bg=BG3)
        row.pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(row, text="Folder", bg=BG3, fg=TEXT_DIM).pack(side="left", padx=(0, 6))
        entry = tk.Entry(row, textvariable=folder_var)
        entry.pack(side="left", fill="x", expand=True, padx=(0, 6))

        def browse():
            from tkinter import filedialog
            f = filedialog.askdirectory(parent=dlg, title=title, initialdir=folder_var.get() or root)
            if f:
        ...
        tk.Label(dlg,
                 text="Or click a folder in the tree below, or click a folder in the main FTmod folder tree:",
                 bg=BG3, fg=TEXT_DIM, font=("Segoe UI", 9)).pack(anchor="w", padx=10, pady=(0, 4))

        tree_frame = tk.Frame(dlg, bg=BG3)
        tree_frame.pack(fill="both", expand=True, padx=10, pady=(0, 8))
        dest_tree = ttk.Treeview(tree_frame, show="tree")
        sb = ttk.Scrollbar(tree_frame, orient="vertical", command=dest_tree.yview)
        dest_tree.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        dest_tree.pack(side="left", fill="both", expand=True)

        def add_node(parent, path, text):
            path = _ui_path(path)
            if dest_tree.exists(path):
                return
            dest_tree.insert(parent, "end", iid=path, text=text, open=False)
            try:
                for e in os.scandir(_longpath(path)):
                    if e.is_dir(follow_symlinks=False):
                        dest_tree.insert(path, "end", iid=path + "|dummy", text="")
                        break
            except Exception:
                pass
        ...
        def populate(path):
            path = _ui_path(path)
            dummy = path + "|dummy"
            if dest_tree.exists(dummy):
                try: dest_tree.delete(dummy)
                except Exception: pass
            try:
                entries = sorted([e for e in os.scandir(_longpath(path)) if e.is_dir(follow_symlinks=False)],
        ...
        if os.path.isdir(_longpath(root)):
            add_node("", root, root)
            dest_tree.item(root, open=True)
            populate(root)

        def on_open(_event=None):
            focus = dest_tree.focus()
            if focus:
                populate(focus)
        ...
        dest_tree.bind("<<TreeviewOpen>>", on_open)
        dest_tree.bind("<<TreeviewSelect>>", on_select)

        buttons = tk.Frame(dlg, bg=BG3)
        ...
        def ok():
            ...
            if not folder or not os.path.isdir(_longpath(folder)):
    ...
    def _context_copy_selected(self, files):
        files = [p for p in (files or []) if os.path.isfile(_longpath(p))]
        if not files:
            messagebox.showinfo("Copy selected", "No files are selected.", parent=self.win)
            return
        dest = self._context_choose_destination("Copy selected files")
        if not dest:
            return
        try:
            result = ft_file_ops.copy_files(files, dest, overwrite=False)
            # Copy thumbnail blobs for any copied files so FTmod's DB remains in sync.
            copied_thumb_items = []
            ...
            self._context_show_file_op_result("Copy selected", result)
        except Exception as e:
            messagebox.showerror("Copy selected", str(e), parent=self.win)

    def _context_move_selected(self, files):
        files = [p for p in (files or []) if os.path.isfile(_longpath(p))]
        if not files:
            messagebox.showinfo("Move selected", "No files are selected.", parent=self.win)
            return
        dest = self._context_choose_destination("Move selected files")
        if not dest:
            return
        try:
            ...
            self._context_finish_file_op(result, extra_folders=[dest, getattr(self, "current_folder", "")])
            self._context_show_file_op_result("Move selected", result)
        except Exception as e:
            messagebox.showerror("Move selected", str(e), parent=self.win)

    def _context_delete_selected(self, files):
        files = [p for p in (files or []) if os.path.isfile(_longpath(p))]
        if not files:
            messagebox.showinfo("Delete selected", "No files are selected.", parent=self.win)
            return
        if not messagebox.askyesno("Delete selected", f"Delete {len(files)} selected file(s) from disk?", parent=self.win):
            return
        try:
            result = ft_file_ops.delete_files(files)
            self._context_update_state_after_deletes(result.deleted)
            for folder in sorted({os.path.dirname(p) for p in result.deleted if p}, key=str.lower):
                try: thumb_gc(folder)
                except Exception: pass
            self._context_finish_file_op(result, extra_folders=[getattr(self, "current_folder", "")])
            self._context_show_file_op_result("Delete selected", result)
        except Exception as e:
            messagebox.showerror("Delete selected", str(e), parent=self.win)

    def _context_update_state_after_moves(self, moved_pairs):
        for old, new in moved_pairs:
            if old in self._selected:
                self._selected.discard(old)
            if old in self._placed_set:
                self._placed_set.discard(old); self._placed_set.add(new)
            try:
                i = self._placed.index(old); self._placed[i] = new
            except Exception:
                pass
            if old in self.tagged:
                self.tagged.discard(old); self.tagged.add(new)
                self.tagged_at[new] = self.tagged_at.pop(old, "")
                try:
                    i = self.tagged_order.index(old); self.tagged_order[i] = new
                except Exception:
                    pass
            if old in self._culled:
                self._culled.discard(old); self._culled.add(new)
                self._culled_at[new] = self._culled_at.pop(old, "")
            if old in self._edited:
                self._edited.discard(old); self._edited.add(new)
            try:
                i = self._all_files.index(old); self._all_files[i] = new
            except Exception:
                pass
        try: self._schedule_save()
        except Exception: pass
        try: _write_cull_list(self.mode_cfg['root'], self._culled, self._culled_at)
        except Exception: pass

    def _context_update_state_after_deletes(self, deleted):
        for old in deleted:
            self._selected.discard(old)
            self._placed_set.discard(old)
            try:
                while old in self._placed:
                    self._placed.remove(old)
            except Exception:
                pass
            self.tagged.discard(old)
            self.tagged_at.pop(old, None)
            try:
                while old in self.tagged_order:
                    self.tagged_order.remove(old)
            except Exception:
                pass
            self._culled.discard(old)
            self._culled_at.pop(old, None)
            self._edited.discard(old)
            try:
                while old in self._all_files:
                    self._all_files.remove(old)
            except Exception:
                pass
        try: self._schedule_save()
        except Exception: pass
    ...
    def _context_finish_file_op(self, result, extra_folders=None):
        affected = set(extra_folders or [])
        try:
            affected.update(result.affected_folders())
        except Exception:
            pass
        affected = sorted({os.path.normpath(f) for f in affected if f}, key=str.lower)
        try:
            if getattr(self, "_tree_widget", None) and hasattr(self._tree_widget, "refresh_after_file_ops"):
                self._tree_widget.refresh_after_file_ops(affected)
            else:
                self._schedule_tree_refresh()
        except Exception:
            try: self._schedule_tree_refresh()
            except Exception: pass
        try:
            self._repaint_selection()
            self._update_sel_bar()
            ...
            pass
        cur = os.path.normpath(getattr(self, "current_folder", "") or "")
        if cur:
            try:
                self._load_folder(cur)
            except Exception:
                try: self._reload_page()
                except Exception: pass
        try:
            self.win.after(300, lambda: self._tree_widget.refresh_after_file_ops(affected) if getattr(self, "_tree_widget", None) and hasattr(self._tree_widget, "refresh_after_file_ops") else None)
        except Exception:
            pass

    def _context_show_file_op_result(self, title, result):
        lines = [title, "", f"Completed: {result.ok_count}"]
    ...
    def _show_pdf_ops_menu(self, event, path):
        """Compatibility wrapper: older bindings now open the PDF edit dialog."""
        try:
            if not path or os.path.splitext(path)[1].lower() != ".pdf":
                return "break"
            selected_pdfs = [p for p in getattr(self, "_selected", set())
                             if os.path.splitext(p)[1].lower() == ".pdf"]
            targets = selected_pdfs if path in selected_pdfs else [path]
            self._show_pdf_edit_dialog(targets)
            return "break"
        except Exception as e:
            messagebox.showerror("PDF conversion", f"Could not open PDF dialog:\n\n{e}", parent=self.win)
            return "break"

    def _convert_selected_pdfs(self, paths, mode):
        """Convert one or more PDFs to grayscale or true B/W without modifying originals."""
        paths = [p for p in paths if p and os.path.splitext(p)[1].lower() == ".pdf"]
        if not paths:
            return
        if mode == "split":
            created = []
            ...
            for p in paths:
                try:
                    result = ft_pdf_ops.split_pdf_to_single_pages(p)
                    created.extend(result.output_paths)
                except Exception as e:
                    errors.append(f"{os.path.basename(p)}: {e}")

            try:
                for p in created:
                    if p not in self._all_files and os.path.isfile(p):
                        self._all_files.append(p)
                self._all_files.sort(key=str.lower)
                self._show_page()
                self._update_statusbar()
                try:
                    self._refresh_tree_stats_debounced()
                except Exception:
                    pass
            except Exception:
                try:
            ...
            msg = f"Created {len(created)} page PDF file{'s' if len(created) != 1 else ''}."
            if errors:
                msg += "\n\nSome files failed:\n" + "\n".join(errors)
                messagebox.showwarning("Split PDF", msg, parent=self.win)
            else:
                messagebox.showinfo("Split PDF", msg, parent=self.win)
            return

        mode_label = "Grayscale" if mode == "grayscale" else "B/W"
        if len(paths) > 1:
            msg = f"Create {mode_label} copies of {len(paths)} selected PDFs?\n\nOriginal PDFs will not be modified."
        else:
            msg = f"Create a {mode_label} copy of this PDF?\n\n{os.path.basename(paths[0])}\n\nOriginal PDF will not be modified."
        if not messagebox.askyesno("PDF conversion", msg, parent=self.win):
            return
        ...
        def _worker():
            ...
            def _finish():
                try:
                    affected_folders = sorted({os.path.dirname(p) for p in paths + made})
                    # Ensure new files are visible and counts update.  New thumbnail DB rows
                    # will be generated by the normal FTmod thumbnail loader when the folder reloads.
                    try:
                        if getattr(self, "_tree_widget", None) and hasattr(self._tree_widget, "refresh_after_file_ops"):
                            self._tree_widget.refresh_after_file_ops(affected_folders)
                        else:
                            self._schedule_tree_refresh()
                    except Exception:
                        self._schedule_tree_refresh()
                    ...
                    cur = _ui_path(getattr(self, "current_folder", ""))
                    if cur and any(_ui_path(f) == cur for f in affected_folders):
                        try:
                            self._load_folder(cur)
                        except Exception:
                            pass
                    ...
                    if errors:
                        detail = "\n".join(f"{os.path.basename(p)}: {err}" for p, err in errors[:8])
                        if len(errors) > 8:
                            detail += f"\n... and {len(errors)-8} more"
                        messagebox.showwarning(
                            "PDF conversion finished",
                            f"Created {len(made)} {mode_label} PDF(s).\n\nErrors:\n{detail}",
                            parent=self.win,
                        )
                    else:
                        messagebox.showinfo(
                    ...
                    try:
                        self._status(f"Created {len(made)} {mode_label} PDF(s)")
                    except Exception:
                        pass
                except Exception as e:
                    messagebox.showerror("PDF conversion", f"Refresh failed after conversion:\n\n{e}", parent=self.win)
    ...
    def _on_thumb_click(self, event, orig, idx):
        """FTView-style thumb click handling.

        Click shows the image and syncs the file list. Ctrl-click toggles operation
        selection. Shift-click repeats the previous select/deselect action across a range.
        """
        try:
            self._show_preview_for_path(orig)
        except Exception:
            pass
        ctrl  = (event.state & 0x4) != 0
        shift = ((event.state & 0x1) != 0 or (event.state & 0x100) != 0) and not ctrl
        ...
        if ctrl:
            ...
            visible_files = {item[2] for item in self.thumb_widgets if item[0].winfo_ismapped()}
            action = getattr(self, '_last_select_action', 'select')
            for i in range(lo, hi + 1):
                if 0 <= i < len(self._all_files):
                    f = self._all_files[i]
                    if f in visible_files:
                        if action == "deselect":
                            self._selected.discard(f)
                        else:
                            self._selected.add(f)
            ...
            # Plain click is preview/navigation only. Do NOT update _last_click_idx
            # so Shift+click always ranges from the previous Ctrl+click anchor.
            pass
        ...
        for item in self.thumb_widgets:
            ...
            try:
                if two_panel:
                    ...
                    cv.delete("moved_watermark")
                    cv.delete("moved_watermark_bg")
                    cv.delete("del_watermark")
                    cv.delete("del_watermark_bg")
                    # In 2-panel mode, Ctrl/Shift selection in the left panel uses
                    # SELECTED wallpaper.  After Move Right, the same left-panel
                    # thumbnails remain visible but change to MOVED wallpaper until
                    # moved back or 2-panel mode ends.
                    bbox = cv.bbox("debug_img_box") or cv.bbox("thumb_img")
                    if bbox:
                        x0, y0, x1, y1 = bbox
                        WM_H = 28
                        mid_y = (y0 + y1) // 2
    ...
    def _two_panel_build_right(self):
        ...
        # Right-panel selected items use the same shared FTView thumbnail
        # metrics as the left/single-panel thumbs.  They are smaller only
        # because the right panel has less available width.
        self._right_canvas.update_idletasks()
        rw = self._right_canvas.winfo_width() or self._right_frame.winfo_width() or 500
        rh = self._right_canvas.winfo_height() or self.canvas.winfo_height() or 500
        avail_w = max(120, int(rw) - 17)
        avail_h = max(120, int(rh))
        base_img = max(80, int(getattr(self, "_disp_size", THUMB_SIZE) or THUMB_SIZE))
        base_cell_w = base_img + 10
        cols = max(1, int((avail_w - 6) // max(1, base_cell_w + 6)))
        r_layout = calculate_thumb_layout(
            panel_width=avail_w,
            panel_height=avail_h,
            item_count=max(1, n),
            columns=cols,
        ...
        cols    = int(r_layout.columns)
        SZ      = int(r_layout.image_w_px)
        IMG_H   = int(r_layout.image_h_px)
        CW      = int(r_layout.cell_w_px)
        CH      = int(r_layout.cell_h_px)
        CTL_H   = 18
        CAP_H   = 20                   # caption area — single line
        STRIP_H = 15
        BD      = 3
        RPAD    = int(r_layout.boundary_gap)
        IX      = int(r_layout.image_x_px)
        IY      = int(r_layout.image_y_px)
        CTL_Y   = IY + IMG_H
        CAP_Y   = CTL_Y + CTL_H + 3
        sy      = CAP_Y + CAP_H
        PAD     = int(r_layout.gap) // 2
        ...
        for idx, fpath in enumerate(self._placed):
            ...
            # Sequence badge
            cv.create_rectangle(IX, IY, IX + 39, IY + 24, fill="#000", outline="")
            cv.create_text(IX + 4, IY + 12, anchor="w", text=str(idx + 1), fill="#aaccff",
                           font=("Segoe UI", 12, "bold"))
            ...
            _fext = os.path.splitext(fpath)[1].lower()
            icon  = "📷 " if _fext in PHOTO_EXTS else ("🎬 " if _fext in VIDEO_EXTS else "📄 ")
            cv.create_text(IX + 2, CAP_Y + 2, anchor="nw",
            ...
            def _load(cv=cv, fp=fpath, sz=SZ, img_h=IMG_H, cw=CW, iy=IY):
                try:
                    ...
                    def _place(tk_img=tk_img, cv=cv, cw=cw, sz=sz, img_h=img_h, iy=iy, fp=fp):
                        try:
                            cv._tk_img = tk_img
                            cv.create_image(cw // 2, iy + img_h // 2,
                                            image=tk_img, anchor="center", tags="thumb_img")
                            if fp in self._right_sel:
                                WM_H = 28
                                ymid = iy + img_h // 2
                                x0 = (cw - sz) // 2
                                x1 = x0 + sz
            ...
            def _rclick(e, fp=fpath, cv=cv, i=idx, cw=CW, iz=INS_ZONE):
                ...
                # Update only visible borders and MOVE wallpaper — no rebuild
                for path, widget in getattr(self, "_right_cvs", {}).items():
                    try:
                        ...
                        widget.delete("move_watermark")
                        widget.delete("move_watermark_bg")
                        if sel:
                            bbox = widget.bbox("thumb_img")
                            if bbox:
                                x0, y0, x1, y1 = bbox
                                WM_H = 28
                                mid_y = (y0 + y1) // 2
    ...
    def _two_panel_move_right(self):
        ...
        for f in to_move:
            self._selected.discard(f)
        self._last_click_idx = None
        # Change the left-panel wallpaper for moved items from SELECTED to MOVED.
        # Right-panel selection continues to use MOVE wallpaper.
        self._repaint_selection(changed_only=set(to_move))
        self._two_panel_build_right()
        self._update_sel_bar()
    ...
    def _show_selection_panel(self):
        ...
        if not getattr(self, '_sel_panel', None) or not self._sel_panel.winfo_exists():
            ...
            CATEGORIES = [
                ...
                ("PDF Merge Queue", [
                    ("➕  Add selected PDFs", self._add_to_pdf_merge_queue),
                    ("📂  Open Merge Queue", self._open_pdf_merge_queue),
                    ("🔗  Merge Queue", self._merge_pdf_merge_queue),
                    ("🧹  Clear Merge Queue", self._clear_pdf_merge_queue),
                ]),
                ("Rotate / Flip", [
    ...
    def _ops_contact_sheet(self):
        """Toolbar/Operations entry for Contact Sheet.

        Keep the button-strip and Operations-panel paths identical and direct.
        The shared ft_contactsheet dialog chooses selected files first, then
        tagged files, then currently displayed files.
        """
        return self._contact_sheet_dialog()
    ...
    def _update_tagged_btn(self):
        """Update legacy toolbar state without blocking Contact Sheet.

        Contact Sheet can be built from selected files, tagged files, or the
        currently displayed folder/list.  The old button-strip logic only
        enabled the button when a tagged collection existed, which left the
        toolbar button greyed out even though the Operations path worked.
        """
        try:
            # Keep Contact Sheet always callable.  The shared dialog already
            # handles the no-source case with a clear message, and disabling this
            # button caused the toolbar to look broken even when the right-click
            # path worked.
            if self.btn_contact:
    ...
    def _rotate_thumb(self, path, lbl_widget, degrees):
        """Silently rotate source image and refresh the visible thumbnail safely.

        The thumbnail database must always contain the standard stored thumbnail
        generated from the source file, not the current display-sized canvas image.
        Also, Tk PhotoImage objects must be created on the Tk/main thread only.
        """
        try:
            from PIL import ImageOps as _IOS, ExifTags as _ET
            ...
            # Source changed: clear stale size and in-memory PhotoImage cache.
            _size_cache.pop(path, None)
            try:
                for key in list(_photo_cache.keys()):
            ...
            # Rebuild the normal DB thumbnail using the same code path as ordinary
            # thumbnail generation.  This avoids storing a display-sized/cropped
            # thumbnail that later renders as zoomed or distorted.
            jpeg_bytes = _thumb_make_jpeg_from_path(
                path,
                THUMB_STORE_SIZE,
                longpath_func=_longpath,
    ...
    def _make_thumb(self, path):
        ...
        try:
            ext = os.path.splitext(path)[1].lower()
            if ext in VIDEO_EXTS and HAVE_VIDEO_THUMB:
                img, _ok, _err = _make_video_thumb(path, 0.1, THUMB_STORE_SIZE, _longpath)
                if img is None:
                    return None
                buf = _io.BytesIO()
                img.save(buf, "JPEG", quality=82)
                jpeg_bytes = buf.getvalue()
                thumb_put_many([(path, jpeg_bytes)])
                return _thumb_jpeg_to_image(jpeg_bytes)
                ...
                if not ok or img is None:
                    raise RuntimeError(err or "DOCX preview failed")
                buf = _io.BytesIO()
                img.save(buf, "JPEG", quality=82)
                jpeg_bytes = buf.getvalue()
            else:
    ...
    def _refresh_thumb_cell(self, orig_path):
        ...
        IMPORTANT: this must use the same display sizing as the normal
        thumbnail loader (_add_cell_from_img).  The canvas image area is
        shorter than it is wide, so fitting to a square makes rotated portrait
        images appear zoomed/clipped until the whole folder is regenerated.
        """
        try:
            use_stored = True
            try:
            ...
            if use_stored:
                jpeg = thumb_get(orig_path) if use_stored else None
                if not jpeg:
                    return
                ImageFile.LOAD_TRUNCATED_IMAGES = True
                ...
                img, ok, _err = _thumb_get_thumbnail(orig_path, sz, longpath_func=_longpath)
                if not ok or img is None:
                    return
            ...
            photo = ImageTk.PhotoImage(img)
            _photo_cache_put(orig_path, sz, photo, 'stored' if use_stored else 'screen')
            for item in self.thumb_widgets:
                if item[2] == orig_path:
                    try:
    ...
    def _on_grid_dims_change(self, changed=None):
        """Apply a user column-count change.

        The column count controls the grid directly.  Thumbnail size is then
        recomputed from the available canvas width, so changing Columns visibly
        changes both the number of columns and the thumbnail size.
        """
        if getattr(self, '_updating_spinners', False):
            return

        try:
            cols = max(1, int(str(self._cols_var.get()).strip()))
        except Exception:
            cols = max(1, int(getattr(self, '_user_cols', 1) or 1))

        self._user_cols = cols

        # Recompute now so _disp_size reflects the new column count before saving.
        self._compute_grid_dims()

        # Persist so next launch uses the same column count and fitted thumbnail size.
        global DEFAULT_COLS
        ...
        try:
            ...
            for line in raw:
                stripped = line.strip()
                if stripped.lower() == '[display]':
                    in_disp = True
                    continue
                if in_disp and stripped.startswith('['):
                    in_disp = False
                if not in_disp:
                    new_raw.append(line)
            if new_raw and new_raw[-1].strip():
                new_raw.append('\n')
    ...
    def _gen_cache_dialog(self):
        ...
        def _selected_cache_folder():
            try:
                sel = self.tree.selection()
                if sel and "__ph__" not in sel[0] and os.path.isdir(_ui_path(sel[0])):
                    return _ui_path(sel[0])
            except Exception:
                pass
            return _ui_path(getattr(self, 'current_folder', ''))
        folder_var=tk.StringVar(value=_selected_cache_folder())
        ...
        entry.pack(side="left",fill="x",expand=True,padx=(0,6))
        def browse():
            f=fd.askdirectory(title="Select folder",initialdir=_ui_path(folder_var.get()))
            if f: folder_var.set(_ui_path(f)); _refresh_count()
        ...
        def _refresh_count(*_):
            folder=_ui_path(folder_var.get())
    ...
    def _run_gen_cache(self, folder, scope, overwrite):
        """Generate thumbnails without the old modal progress dialog or post-finish freeze."""
        folder = _ui_path(folder)
        exts = self.mode_cfg['exts']
        word = self.mode_cfg['file_word']

        error_list = []
        skipped_list = []
        self._status("Generating thumbnails… counting files in background")

        progress = {"win": None, "label": None, "bar": None, "total": 1}

        def _show_progress(total):
            try:
                pw = tk.Toplevel(self.win)
                pw.title("Generating Thumbnails")
                pw.configure(bg=BG3)
                pw.resizable(False, False)
                pw.transient(self.win)
                self._centre_window(pw, 520, 150)
                tk.Label(pw, text="Generating thumbnails", bg=BG3, fg=TEXT_BRIGHT,
                         font=("Segoe UI", 11, "bold")).pack(pady=(14, 4))
                lbl = tk.Label(pw, text=f"0/{total:,}", bg=BG3, fg=TEXT_BRIGHT,
                               font=("Segoe UI", 9), wraplength=480)
                lbl.pack(padx=16, pady=(0, 8))
                from tkinter import ttk as _ttk
                bar = _ttk.Progressbar(pw, mode="determinate", maximum=max(1, total))
                bar.pack(fill="x", padx=22, pady=(0, 14))
                pw.protocol("WM_DELETE_WINDOW", lambda: None)
                progress.update({"win": pw, "label": lbl, "bar": bar, "total": max(1, total)})
            except Exception:
                progress.update({"win": None, "label": None, "bar": None, "total": max(1, total)})

        def _update_progress(done, total, generated, skipped, errors, path):
            try:
                lbl = progress.get("label")
                bar = progress.get("bar")
                if lbl is not None:
                    lbl.config(text=(f"{done:,}/{total:,} — {generated:,} generated, "
                                     f"{skipped:,} skipped, {errors:,} errors\n"
                                     f"{os.path.basename(path)}"))
                if bar is not None:
                    bar["maximum"] = max(1, total)
                    bar["value"] = done
                win = progress.get("win")
                if win is not None:
            ...
            if scope == "tree":
                ...
                try:
                    for r, d, fs in os.walk(folder):
                        ...
                        for f in sorted(fs, key=str.lower):
                            if os.path.splitext(f)[1].lower() in exts:
                                result.append(os.path.join(r, f))
                except Exception as ex:
                    error_list.append((os.path.basename(folder), f"Folder scan error: {ex}"))
                return result
            try:
                return sorted(
        ...
        def _finish(gen, skipped, status, total, bytes_generated=0):
            n_err = len(error_list)
            try:
                if progress.get("win") is not None:
                    progress["win"].destroy()
            except Exception:
                pass
            self._status(f"Thumbnails {status.lower()}: {gen} generated, {skipped} skipped, {n_err} errors")

            # Critical: do not call _load_folder(), refresh_stats(), or repeated
            # tree refreshes here.  Those were the source of the post-generation
            # Not Responding delay.  Only refresh the selected/current visible
            # tree rows' fast Files/Thumbs columns.
            try:
                cur = _ui_path(getattr(self, 'current_folder', '') or '')
                if cur and self.tree.exists(cur):
                    self._tree_widget._fill_node(cur, skip_tree_col=True)
                sel = self.tree.selection()[0] if self.tree.selection() else ''
                if sel and sel != cur and self.tree.exists(sel):
                    self._tree_widget._fill_node(sel, skip_tree_col=True)
            except Exception:
                pass

            rdlg = tk.Toplevel(self.win)
            rdlg.title("Generate Thumbnails — " + status)
            rdlg.configure(bg=BG3)
            rdlg.transient(self.win)
            rdlg.grab_set()
            ...
            tk.Label(rdlg, text=f"Files:      {total:,}", bg=BG3, fg=TEXT_BRIGHT,
                     font=("Segoe UI", 10)).pack()
            tk.Label(rdlg, text=f"Generated:  {gen:,}", bg=BG3, fg=TEXT_BRIGHT,
                     font=("Segoe UI", 10)).pack()
            tk.Label(rdlg, text=f"Bytes:      {bytes_generated:,} bytes ({bytes_generated/1024/1024:.2f} MB)",
            ...
            def show_list(title, items, colour):
                ...
                ed.title(title)
                ed.configure(bg=BG3)
                self._centre_window(ed, 700, 420)
                tk.Label(ed, text=title, bg=BG3, fg=colour,
                         font=("Segoe UI", 10, "bold")).pack(pady=(12, 4), padx=16, anchor="w")
                fr2 = tk.Frame(ed, bg=BG3)
                fr2.pack(fill="both", expand=True, padx=16, pady=4)
                sb2 = tk.Scrollbar(fr2)
                ...
                for item in items:
                    if isinstance(item, tuple):
                        ...
                        lb.insert("end", item)
                tk.Button(ed, text="  Close  ", bg=BG2, fg=TEXT_BRIGHT,
                          font=("Segoe UI", 9), relief="flat",
                          command=ed.destroy).pack(pady=8)
            ...
            def _close():
                try: rdlg.grab_release()
                except Exception: pass
                rdlg.destroy()

            tk.Button(bf2, text="  Close  ", bg=BG2, fg=TEXT_BRIGHT, font=("Segoe UI", 10),
                      relief="flat", padx=10, cursor="hand2", command=_close
                      ).pack(side="left", padx=4)
            rdlg.protocol("WM_DELETE_WINDOW", _close)
        ...
        def worker():
            ...
            if total == 0:
                self.win.after(0, lambda: messagebox.showinfo(
                    f"No {word}s", f"No {word} files found.", parent=self.win))
                self.win.after(0, lambda: self._status(f"No {word} files found"))
                return
            ...
            self.win.after(0, _show_progress, total)

            for cur_folder, folder_files in folder_groups.items():
                pending_items = []
                for orig in folder_files:
                    ...
                    if already is not None:
                        ...
                        try:
                            ...
                            if ext in DOCX_EXTS:
                                img, ok, err = make_preview_thumbnail(
                                    orig, THUMB_STORE_SIZE, longpath_func=_longpath)
                                if not ok or img is None:
                                    raise RuntimeError(err or "DOCX preview failed")
                                buf = _io.BytesIO()
                                img.save(buf, "JPEG", quality=82)
                                ...
                                if not HAVE_VIDEO_THUMB:
                                    raise RuntimeError("ft_movie library not available for video thumbnails")
                                img, ok, err = _make_video_thumb(
                                    orig, 0.1, THUMB_STORE_SIZE, _longpath)
                                if not ok or img is None:
                                    raise RuntimeError(err or "Video thumbnail generation failed")
                                buf = _io.BytesIO()
                                img.save(buf, "JPEG", quality=82)
                                jpeg_bytes = buf.getvalue()
                            else:
                                jpeg_bytes = _thumb_make_jpeg_from_path(
                                    orig, THUMB_STORE_SIZE, longpath_func=_longpath, quality=82)
                            pending_items.append((orig, jpeg_bytes))
                            generated += 1
                            generated_bytes += len(jpeg_bytes or b"")
                        except Exception as ex:
                    ...
                    if done == 1 or done % 20 == 0:
                        self.win.after(
                            0,
                            lambda d=done, t=total, g=generated, s=skipped, e=len(error_list), p=orig:
                                (self._status(
                                    f"Generating thumbnails {d:,}/{t:,} — "
                                    f"{g} generated, {s} skipped, {e} errors — {os.path.basename(p)}"
                                ), _update_progress(d, t, g, s, e, p))
                        )
                ...
                if pending_items:
                    try:
                        ...
                        error_list.append((os.path.basename(cur_folder), f"Blob write error: {ex}"))

            self.win.after(0, _finish, generated, skipped, "Complete", total, generated_bytes)
    ...
    def _clean_orphans_dialog(self):
        ...
        def finish_scan(scanned,orphans):
            ...
            if _db() is not None:
                try:
                    coll_updated = ft_db.cleanup_missing_collection_items(_db(), exists_func=os.path.exists)
                except Exception as e:
    ...
    def _copy_move_dialog(self, files=None):
        ...
        def _dest_tree_expand(event):
            iid = dest_tree.focus()
            if not iid or "__ph__" in iid: return
            ph = iid + getattr(self._tree_widget, "PLACEHOLDER", "__ph__")
    ...
    def _contact_sheet_sort_key(self):
        return ft_contactsheet.contact_sheet_sort_key(self)

    def _contact_sheet_root(self):
        """Active Photos/Documents root for shared contact-sheet output."""
        try:
            return _ui_path(self.mode_cfg.get('root', ''))
        except Exception:
            return self.mode_cfg.get('root', '') if getattr(self, 'mode_cfg', None) else ''

    def _contact_sheets_under_mode_root(self):
        """Return <active root>\_ContactSheets, never current selected folder."""
        root = self._contact_sheet_root()
    ...
    def _contact_sheet_dialog(self):
        self._ft_contact_sheet_root_func = self._contact_sheet_root
        self._ft_contact_sheets_dir_func = self._contact_sheets_under_mode_root
        self._ft_longpath_func = _longpath
        self._ft_thumb_get_func = thumb_get
        return ft_contactsheet.contact_sheet_dialog(self)

    def _run_contact_sheet(self, files, cols, rows, landscape, title, out_path, dlg, rotate_portrait=False):
        self._ft_longpath_func = _longpath
        self._ft_thumb_get_func = thumb_get
        return ft_contactsheet.run_contact_sheet(self, files, cols, rows, landscape, title, out_path, dlg, rotate_portrait=rotate_portrait)

    def _contact_sheet_done(self, out_path, n_images, n_pages, prog=None):
        return ft_contactsheet.contact_sheet_done(self, out_path, n_images, n_pages, prog)
    # ── About ──────────────────────────────────────────────────────────────────
    def _switch_project(self):
        ...
        def _on_select(name, proj):
            ...
            for rpath, _ in proj.get('roots', []):
                if rpath: _migrate_txt_to_db(rpath)
            # Reload UI
            self._clear_grid()
            self._clear_tree()
            self._active_root_idx = 0
            self.mode_cfg = _mode_cfg(self.mode)
            self._update_filter_buttons()
            self._update_root_combobox()
            self._init_mode()
    ...
    def _show_about(self):
        ...
        tk.Label(inner,text="📷 Photos  +  📄 Documents  —  Tag, Browse, Export",
                 bg=SBG,fg=SSUB,font=("Segoe UI",11)).pack(pady=(4,0))
        tk.Label(inner,text=_get_build_string(),bg=SBG,fg=SDIM,font=("Segoe UI",13)).pack(pady=(6,0))
        tk.Label(inner,text=f"Photos:  {PHOTOS_ROOT}",bg=SBG,fg=SDIM,font=("Segoe UI",10)).pack(pady=(2,0))
        tk.Label(inner,text=f"Documents:    {PDFS_ROOT}",bg=SBG,fg=SDIM,font=("Segoe UI",10)).pack(pady=(2,0))
    ...
    def _show_db_status(self):
        ...
        # DB connection state
        state = "Connected" if _db() is not None else "Not connected"
        state_col = SSUB if _db() is not None else "#cc2222"
    ...
    def _row_height_px(self):
        """Pixel height of one complete thumbnail row including grid gap."""
        layout = getattr(self, '_thumb_layout', None)
        if layout is not None:
            return int(layout.cell_h_px + layout.gap)
        sz = int(getattr(self, '_disp_size', THUMB_SIZE) or THUMB_SIZE)
        return sz + 24 + THUMB_PAD
    ...
    def _scroll_rows(self, direction):
        ...
        try:
            # Get scrollregion height — do NOT call update_idletasks() here;
            # calling it on every wheel event forces a full layout flush while
            # cells are still rendering, causing the visible scroll pauses.
...
def _show_project_selector(parent, on_select_cb):
    ...
    tk.Label(dlg, text="Choose an existing project, open another project folder, or create a new one.",
    ...
    def _project_status(proj):
        """Return (short_status, details_text, colour) for the selector list/status panel."""
        path = proj.get('path', '')
        if not path or not os.path.isdir(path):
            return ("MISSING FOLDER",
                    "Project folder is missing or has been deleted. Use Open Existing Project... to point this entry at the correct FTProj_ folder.",
                    "#888888")

        roots = proj.get('photos_roots', []) + proj.get('pdfs_roots', [])
        missing_roots = []
        for root_path, root_name in roots:
            if root_path and not os.path.isdir(root_path):
                missing_roots.append(root_name or root_path)

        if missing_roots:
            return ("ROOTS WARNING",
                    "Project folder exists, but these configured roots are not currently accessible: " + ", ".join(missing_roots),
                    "#ffcc44")

        return ("Online", "Project folder and configured roots are accessible.", TEXT_BRIGHT)

    def _refresh_list(select_name=None):
        lb.delete(0, "end")
        for name in proj_names:
            proj = projects[name]
            short_status, _details, colour = _project_status(proj)
            status = "" if short_status == "Online" else f"  [{short_status}]"
            marker = "★ " if name == active else "  "
            lb.insert("end", f"{marker}{name}{status}")
            if name == active and short_status == "Online":
                lb.itemconfig("end", fg="#ffcc44")
            else:
                lb.itemconfig("end", fg=colour)

        if proj_names:
            if select_name in proj_names:
                idx = proj_names.index(select_name)
            elif active in proj_names:
                idx = proj_names.index(active)
            else:
                idx = 0
            lb.selection_clear(0, "end")
    ...
    lbl_status.pack(fill="x", padx=16, anchor="w")
    ...
    def _on_listbox_select(e=None):
        ...
        path = proj.get('path', '')
        roots = proj.get('photos_roots', []) + proj.get('pdfs_roots', [])
        roots_txt = ", ".join(n for _, n in roots) or "none set"
        db_path = _project_db_path(path) if path else ""
        short_status, details, colour = _project_status(proj)
        lbl_status.config(
            fg=colour if short_status != "Online" else TEXT_DIM,
            text=(f"Path: {path or '(not set)'}\n"
                  f"DB: {'exists' if db_path and os.path.exists(db_path) else 'new'}  |  "
                  f"Status: {short_status}  |  Roots: {roots_txt}\n"
                  f"{details}"))
    ...
    def _open():
        ...
        name = proj_names[sel[0]]
        proj = projects[name]
        if not os.path.isdir(proj.get('path', '')):
            messagebox.showerror(
                "Project Folder Missing",
                "The saved project folder no longer exists:\n\n"
                f"{proj.get('path', '')}\n\n"
                "Use Open Existing Project... to select the correct FTProj_ folder, "
    ...
    def _normalise_project_name(folder):
        base = os.path.basename(os.path.normpath(folder))
        if base.lower().startswith("ftproj_"):
            base = base[7:]
        return base or "Project"

    def _open_existing_project():
        from tkinter import filedialog
        start_dir = os.path.dirname(os.path.abspath(__file__))
        folder = filedialog.askdirectory(
            parent=dlg,
            title="Open Existing FT Project Folder",
        ...
        base = os.path.basename(os.path.normpath(folder))
        if not base.lower().startswith("ftproj_"):
            ok = messagebox.askyesno(
                "Folder is not named FTProj_...",
                "This folder is not named like an FT project folder:\n\n"
                f"{folder}\n\n"
                "Use it as an existing project anyway?",
                parent=dlg)
            if not ok:
                return

        name = _normalise_project_name(folder)

        # If the same project name already exists, update its path rather than
        # creating a duplicate. Preserve its existing roots/settings.
        if name in projects:
            proj = dict(projects[name])
            proj['path'] = folder
        else:
            proj = {'path': folder, 'photos_roots': [], 'pdfs_roots': []}
            proj_names.append(name)

        # If another old/missing entry points at the same project name only by
        # case/spacing, remove the stale duplicate from the visible list.
        for old_name in list(projects.keys()):
            if old_name != name and old_name.lower() == name.lower():
                projects.pop(old_name, None)
                if old_name in proj_names:
                    proj_names.remove(old_name)
    ...
    def _new():
        def _after_create(name, proj):
            projects.update({name: proj})
            if name not in proj_names:
                proj_names.append(name)
            _refresh_list(select_name=name)
        _show_new_project_dialog(dlg, _after_create)
    ...
    def _discover():
        ...
        for f in found:
            if f['name'] not in projects:
                ...
                added += 1
            elif not os.path.isdir(projects[f['name']].get('path', '')):
                projects[f['name']]['path'] = f['path']
                updated += 1
        if added or updated:
            _save_projects(projects)
            _refresh_list()
            messagebox.showinfo("Discover", f"Found {added} new project(s); repaired {updated} missing project path(s).", parent=dlg)
        else:
    ...
    tk.Button(bf, text="  Open  ", bg=GREEN, fg="white",
              ...
              cursor="hand2", command=_open).pack(side="left", padx=5)
    tk.Button(bf, text="  Open Existing Project...  ", bg="#4477aa", fg="white",
              font=("Segoe UI", 10), relief="flat", padx=12, pady=6,
              cursor="hand2", command=_open_existing_project).pack(side="left", padx=5)
    tk.Button(bf, text="  New Project  ", bg="#225588", fg="white",
              font=("Segoe UI", 10), relief="flat", padx=12, pady=6,
              cursor="hand2", command=_new).pack(side="left", padx=5)
    tk.Button(bf, text="  Discover  ", bg="#446644", fg="white",
              font=("Segoe UI", 10), relief="flat", padx=12, pady=6,
              cursor="hand2", command=_discover).pack(side="left", padx=5)
...
def _settings_dialog(parent_win, on_save_cb=None, startup=False):
    ...
    # Keep the dialog usable on high-DPI and smaller screens; the old fixed
    # geometry could open oddly sized/off-centre.
    w = min(820, max(680, sw - 160))
    h = min(760, max(620, sh - 140))
    ...
    # Display size remains user-selectable; cached thumbnails are always stored
    # at THUMB_STORE_SIZE (250x250) until this is deliberately changed later.
    tk.Spinbox(sz_row, from_=100, to=500, increment=50, textvariable=sz_var,
               width=6, bg=BG2, fg=TEXT_BRIGHT, buttonbackground=BG2,
               font=("Segoe UI",9)).pack(side="left")
    tk.Label(sz_row, text=f"px  (stored thumbs: {THUMB_STORE_SIZE}x{THUMB_STORE_SIZE})", bg=BG3, fg=TEXT_DIM,
                ...
                if active_name:
                    ...
                    if active_name in projects:
                        ...
                        try:
                            ...
                            _ACTIVE_PROJECT['name'] = active_name
                        except Exception:
                            pass
            except Exception as _e:
                print(f"settings project save warning: {_e}")
        except Exception as e:
    ...
    for line in text.splitlines():
        ...
        while idx > 1:
            ...
            if candidate_name and len(candidate_path) > 2:
                roots.append((os.path.normpath(candidate_path), candidate_name[:30]))
                break
